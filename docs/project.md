...
Исправление (2025‑09‑15): коэффициенты в «Прогнозах» при появлении матчей за 6 дней
- Причина: при отдаче туров из БД‑снапшота `betting-tours` сервер пересчитывал только `odds` для рынка 1x2, но не формировал `markets` (totals/specials). В сочетании с клиентским кэшем это приводило к карточкам без коэффициентов после 304.
- Сделано:
    - В маршруте `GET /api/betting/tours` (ветка чтения снапшота) теперь всегда формируются и `odds`, и `markets` (тоталы 3.5/4.5/5.5, пенальти, красная) — как в on‑demand билдере.
    - На клиенте (`static/js/predictions.js`) добавлен безопасный форс‑рефетч без ETag, если пришёл `304`, а в локальном кеше отсутствуют коэффициенты/рынки.
    - Дополнительно: даже при неизменном ETag (та же версия) клиент применяет полученные коэффициенты к уже отрисованным карточкам, чтобы они появились сразу при первом заходе на вкладку.
- Итог: при открытии «Прогнозов» сразу отображаются коэффициенты П1/Х/П2 и дополнительные рынки, даже если данные берутся из старого снапшота.
...
Расширение (2025‑09‑15): Fallback поиска логотипов команд
- Модуль `static/js/team-utils.js` расширен: теперь при установке логотипа перебираются варианты `фк<norm>.png|webp`, `<norm>.png|webp`, вариант с дефисами, а также «сырые» формы имени (`raw`, `raw_underscored`, `rawcollapsed`) в форматах PNG/WebP. Это устраняет пропажу логотипов при расхождении именования файлов и нормализации.
- Все успешные URL кэшируются в `_logoCache`, предотвращая повторные проверки `Image.onload/onerror` и снижая нагрузку.
- Если ни один вариант не найден — используется `default.png` (placeholder без мерцаний).
...
- Ставки и коэффициенты
    ...
    - Клиент (ставки): `static/js/predictions.js` (WS + ETag‑fallback пуллинг 3.5–4.7с)
     - Клиент (ставки): `static/js/predictions.js` (WS + ETag‑fallback пуллинг 3.5–4.7с)
     - Тихая предзагрузка: `static/js/predictions-preload.js` — на старте SPA фоном запрашивает `/api/betting/tours` (ETag-aware) и прогревает локальный кэш. При ответе 304 с «пустым» кэшем запускает единичный refetch без If-None-Match и аккуратно мержит odds/markets из кэша, чтобы никогда не затирать их пустыми значениями. UI не трогает.
...
- Сезонный reset и составы команд
    ...
    - Публичные составы: `app.py` → `GET /api/match/lineups?match_id=...`
        - Генерация расписания сезона: `app.py` → `POST /api/admin/schedule/generate`
            ...
            - Поведение: `soft_start` вернёт 409, если уже есть матчи в активном турнире (ответ включает `tournament_id` и `count`); `full_reset` — выполняет полный сброс приложения (как `POST /api/admin/full-reset`, включая очистку ставок и админ‑логов), затем полностью перезаписывает матчи текущего активного турнира и генерирует новый сезон.
