...
- Обновления (2025-09-22): исправлена вставка статистики при переносе — теперь в `team_stats_<id>` всегда пишется корректный `player_id` (id новой записи из `team_roster`), что устраняет ошибку NOT NULL; UI стабилизирован в виде двух колонок (левая — список, правая — очередь), поправлены media‑запросы и лишняя закрывающая скобка в `database-ui.css`.
