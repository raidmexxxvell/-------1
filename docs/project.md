...
Исправление (2025‑09‑15): влияние голосов на коэффициенты при размещении ставки
- Симптом: после голосования коэффициенты в разделе «Мои ставки» оставались прежними, если у матча не было точного времени (только дата).
- Причина: при расчёте коэффициентов в момент ставки функция `_compute_match_odds` получала `date_key=None`, и влияние голосов не применялось.
- Решение: в обработчике `POST /api/betting/place` дополнительно сохраняется `match_date_key` (YYYY‑MM‑DD) из расписания даже при отсутствии времени; при расчёте рынка `1x2` в `_compute_match_odds` передаётся `date_key = match_dt.date() или match_date_key`.
- Эффект: голоса начинают корректно влиять на коэффициенты, зафиксированные в ставке, в любых сценариях.
