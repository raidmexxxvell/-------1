...
- Попытка сопоставить `player_id` выполняется через поиск в `team_roster` по (team, lower(player)); если не найдено — детерминированный hash fallback (временная мера до нормализации Players)
