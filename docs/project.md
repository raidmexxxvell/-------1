...
> Текущая версия `app.py`: ~9750 строк (etag_json расширен, /api/match-details переведён на etag_json, централизованный DB retry helper для чтений, добавлены ETag-метрики, cap для лидербордов, X-Updated-At и admin refresh).  
> Последние ключевые изменения: persistent roster (`team_roster`), публичный эндпоинт lineups, унификация match-details через `etag_json` (server) и `fetchMatchDetails` (client), миграция schedule/results на `etag_json` (server) и `fetchEtag` (client), числовой прогресс splash (stage API), UI фиксы горизонтального скролла, централизованный DB retry для `/api/betting/my-bets`, перенос «Цели» в разделе достижений внутрь «Подробнее», оркестрация скрытия splash по событиям предзагрузки (news/ads/topmatch), снижение шума логов для отсутствующих матчей в расчёте тоталов (INFO один раз на пару команд).

Примечание (2025‑09‑08): при `POST /api/match/status/set` со статусом `finished` сервер теперь сначала апсертит финальный счёт из `MatchScore` в snapshot `results` и авто‑фиксирует спецрынки (penalty/redcard) как «Нет» при отсутствии явной отметки, затем выполняет расчёт ставок и немедленно синхронизирует турнирную таблицу. Это устраняет кейс, когда открытые ставки зависали при завершении матча без ручного `/api/match/settle`.
