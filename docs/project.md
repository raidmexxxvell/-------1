...
> Текущая версия `app.py`: ~9750 строк (etag_json расширен, /api/match-details переведён на etag_json, централизованный DB retry helper для чтений, добавлены ETag-метрики и кап в лидербордах).  
...
- POST `/api/admin/google/export-all` — экспорт актуальных данных в таблицу: пользователи, ставки, турнирная таблица, а также расширенные вкладки статистики:
    ...
    - Источник данных: при наличии расширенной схемы (PlayerStatistics/MatchEvent/TeamComposition) используется она; если нет — включается fallback на события/составы, чтобы не ломать экспорт.
