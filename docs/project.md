...
- entity `odds`: при изменении коэффициентов после ставки пользователя
    ...
    - Доставка: для интерактива теперь используется немедленная отправка (priority=1) в топики `match_odds_{match_id}` и `predictions_page`; общий батч‑дебоунс сохраняется для фоновых/массовых апдейтов.
