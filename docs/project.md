...
- POST `/api/admin/google/export-all` — экспорт актуальных данных в таблицу: пользователи, ставки, турнирная таблица, а также расширенные вкладки статистики:
    ...
    - Источник данных: при наличии расширенной схемы (PlayerStatistics/MatchEvent/TeamComposition) используется она; если нет — включается fallback на `team_roster` с улучшениями: дедуп имён (case-insensitive), разделение full name на `first_name`/`last_name`, попытка сопоставления с `Player.id` по шаблонам «Фамилия Имя» и «Имя Фамилия» (и осторожный contains‑fallback) для заполнения `player_id` и корректного сплита.
