...
2. **Админский UI и трансферы**
   ...
   - Массовый ввод состава перенесён в «Команды → Состав»: вставленный список добавляет новых игроков и перевозит архивных в статус `active`, не затрагивая уже заявленных и их статистику (удаление только через трансфер).
