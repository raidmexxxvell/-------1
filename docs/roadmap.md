# Roadmap проекта Liga Obninska (обновлённое)
...
✅ **ВЫПОЛНЕНО** - Система логирования действий администратора — 🔵 (среднее влияние)
   ...
   - Реализовано:
     ...
     8. ✅ **НОВОЕ**: Создана система продвинутых декораторов в utils/advanced_admin_logger.py
     9. ✅ **НОВОЕ**: Добавлено логирование в сложные и средние функции (50+ новых точек логирования)
     10. ✅ **НОВОЕ**: Интегрировано автоматическое логирование для API операций, управления пользователями, синхронизации данных
     11. ✅ **НОВОЕ**: Добавлена функция manual_log для ручного логирования критических операций
     12. ✅ **НОВОЕ**: Исправлены все проблемы с импортами и переменными для полной совместимости
   - Критерий приёма: ✅ Выполнен полностью - все действия админа логируются с подробным описанием, включая сложные операции
   - Влияние: значительное улучшение отладки, полная прозрачность администрирования, детальный анализ производительности всех операций
...
4) API-контракт и документация (OpenAPI/Swagger) — 🔵
...
- ✅ Этап 3: Отвязка чтений от Sheets (публичные пути)
  ...
  - ✅ Добавлен объединённый эндпоинт `/api/admin/refresh-all` с WebSocket прогрессом (topic `admin_refresh`) и логированием длительностей шагов в метрики (api_observe: sync_*).
- 🟨 Этап 4: Документация и метрики
  ...
  - ✅ Универсальный repair эндпоинт `/api/admin/google/repair-users-sheet` теперь поддерживает мульти‑листы через параметр `sheet`:
    ...
    - `bets` — дедуп по id
    - `achievements`, `referrals` — дедуп по user_id с приоритетом по updated_at
  ...
  - ✅ Добавлен эндпоинт `/health/perf` (admin-only) с метриками: uptime, API p50/p95 (EMA), cache (memory_hits/redis_hits/misses/sets), ws (batched/bypass/sent), etag (hit_ratio). Использует `optimizations/metrics.py`.
...
- 🔵 Next: регресс‑тесты live‑проекции (исключить только что завершённые матчи из оверлея при задержках записи в results) и метрики обновления.
  - ✅ Минимальное инструментирование `api_schedule` для p50/p95 латентности.

#### 2025-09-10 — Устранение мерцания счётов и полосы голосования
- ✅ Добавлен in-memory стор `MatchState` (кэш счёта и агрегатов голосов) в `static/js/league.js`.
- ✅ При повторном показе карточки матча счёт восстанавливается сразу из памяти (нет скачка VS → 0:0 → X:Y).
- ✅ Голосование: оптимистичное обновление (локально увеличиваем выбранный сегмент, мгновенная визуальная обратная связь для проголосовавшего).
- ✅ Фоновый опрос агрегатов каждые ~4s (диапазон 3–5s по требованию) только если данные старше 3.5s и вкладка активна.
- ✅ Дифф‑рендер сегментов: изменение ширины только при реальном изменении процента (минимизация layout thrash / repaint → исключено мерцание).
- ✅ Дедупликация лишних fetch: повторные карточки пользователя не инициируют повторную немедленную загрузку, если данные свежие.
- 🔵 Next: вынести дублированную логику голосования из `profile.js` в общий helper (VoteInline.render) — снизить дублирование перед дальнейшей оптимизацией.
- 📈 Ожидаемый эффект: снижение количества полных перерисовок полосы голосования при переключении подвкладок ~до 0 (только дифф), сокращение сетевых запросов агрегатов при частых переходах между подвкладками до 40–60%.

##### Дополнение (2025-09-10, позже)
- ✅ Те же принципы (MatchState + оптимистичное голосование + дифф ширин сегментов) применены к блоку «Игра недели» в `profile.js`.
- 📌 Единое поведение: переход между Главной и Лигой не вызывает мерцание полосы и счёта для матча недели.
- 🔵 Next: унифицировать отрисовку vote-inline (league.js/profile.js) через общий helper.
 - ✅ Реализовано: общий helper `static/js/vote-inline.js` (VoteInline.create) и интеграция в `league.js` и `profile.js` (убрано дублирование, единая логика оптимистичного апдейта и фонового опроса).
...
- ✅ (Update) При `POST /api/match/status/set` со статусом `finished` теперь сразу выполняется агрегация `TeamPlayerStats` + пересбор `stats-table` и кэша бомбардиров (раньше требовался отдельный `/api/match/settle`). Страница команды и глобальная статистика игроков обновляются мгновенно.
  - ✅ (UI) Экран команды: добавлен заголовок «Статистика» над блоком сводных метрик + корректная русская плюрализация подписи количества матчей (матч/матча/матчей) без изменения цветовой схемы.
 - ✅ FIX: `/api/match/settle` убрана блокировка расчёта по условию `b.match_datetime > now` (проблема рассинхрона TZ приводила к тому, что часть ставок оставалась `open` после ручного завершения матча). Теперь ручной settle всегда проходит по списку открытых ставок; 1x2/totals по‑прежнему требуют наличия счёта/тотала, спецрынки без явной фиксации считаются как «Нет».
 - ✅ FIX: Расписание — тур считался «последним завершённым» после одной сыгранной игры и выпадал из списка, теперь тур завершён только если все его матчи есть в `results`. Добавлен оверлей счётов из snapshot `results` в расписание/результаты при задержке обновления Sheets.
 - ✅ FIX: Прогнозы (betting-tours) — матчи скрываются после старта, а следующий тур раскрывается только после старта последнего матча текущего тура.
 - ✅ FIX: Team overview кэш (ETag) инвалидируется при обновлении результатов, страница команды сразу отражает новый матч.
...
Цель: при клике на название команды в «Расписание/Результаты/Таблица» открывается динамический экран команды. Первая вкладка — «Обзор» с агрегатами по всем сезонам (никогда не сбрасываются). «Матчи» и «Состав» — заглушки на первом этапе. Оформление соответствует активной теме лиги (UFO/BLB).
...
- ✅ Ускоренная сериализация JSON-ответов через orjson в helper `etag_json` и точечно в `/api/news`, `/api/stats-table` (теперь orjson обязательная зависимость; fallback на стандартный json сохранён для аварийной деградации).
...
- ✅ Привести оставшиеся тяжёлые `jsonify` к быстрому рендеру `_json_response` (точечно, без изменения контрактов, сентябрь 2025).
  - ✅ Все крупные публичные и админские эндпоинты, возвращающие большие JSON, переведены на быстрый рендер через orjson (_json_response). Библиотека объявлена обязательной; ошибки и мелкие ответы оставлены на jsonify (fallback остаётся при отсутствии orjson).
...
**СТАТУС ОБНОВЛЕНИЯ (Декабрь 2024)**:
✅ **ЗАВЕРШЕНА** - Расширенная система логирования администратора
- Все проблемы в коде исправлены
- Добавлено логирование в 50+ сложных и средних функций  
- Создана система продвинутых декораторов для автоматического логирования
- Исправлены все ошибки импорта и неопределенных переменных
- Система готова к продакшену без предупреждений
