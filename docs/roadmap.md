...
1. Stale-While-Revalidate + ETag (универсально)
  ...
  - [~] Match details (/api/match-details) — клиент унифицирован (fetchMatchDetails + fetchEtag), серверный etag_json helper ещё не применён
...
2. Универсальный helper etag_json(endpoint_key, payload_builder)
  ...
  - [ ] Применить к /api/match-details (после нормализации core payload; клиент готов)
...
7. Клиентский SWR слой
  ...
  - [x] Унифицированная утилита fetchEtag(url, cacheKey)
    ...
    - [~] /api/match-details частично (все новые вызовы через fetchMatchDetails; некоторые legacy блоки в процессе замены)
...
1. **Завершить унификацию match-details + leaderboards (fetchEtag + etag_json)**
2. **Alembic миграции (team_roster, подготовка схемы под подписки)**
3. **Sanitize новостей (bleach) + security hardening**
4. **Вынос betting & news из app.py (первый шаг декомпозиции)**
5. **Базовый тестовый каркас (pytest + smoke)**
