...
Ниже — объединённая и актуализированная спецификация topic-based WebSocket routing, server-side batching (debounce/aggregation), delta/patch формата сообщений и безопасного canary-rollout. Этот раздел ранее велся в отдельном `docs/real-time-topic-routing-canary.md` и теперь включён сюда.

Цель: снизить широковещательную нагрузку, доставлять только релевантные дельта‑патчи подписанным клиентам по топикам (`match:{id}`, `team:{id}`, `leaderboard:{type}`), и подготовить канареечный запуск бинарной сериализации (MessagePack) для 5% клиентов.
...
- [x] PR-2: `optimizations/smart_invalidator.py` публикует topic‑сообщения в Redis канал `app:topic` и вызывает локально `websocket_manager.emit_to_topic`.
- [x] PR-2a: добавлены env-флаги `WEBSOCKETS_ENABLED` и `WS_TOPIC_SUBSCRIPTIONS_ENABLED`; frontend делает pre-probe перед Socket.IO и автоподписывается на `match:{home}__{away}__{date}:details` при открытии match-details.
- [x] PR-3: `optimizations/websocket_manager.py` реализует per-topic буферизацию, `emit_to_topic_batched` с агрегацией `data_patch` полей, deep-merge для полей patch, и priority bypass (priority>0 отправляется немедленно). Дефолтный debounce — 180ms.

Изменённые файлы (локально):
- [x] `optimizations/smart_invalidator.py` — `publish_topic`, Redis `app:topic` handling.
- [x] `optimizations/websocket_manager.py` — `emit_to_topic_batched`, topic buffers, metrics (`ws_messages_sent/ws_messages_batched/ws_messages_bypass`).
- [x] `app.py` — админ-эндпоинты (score/events/stats) вызывают `publish_topic(...)` после коммита.
- [x] `static/js/profile-match-advanced.js`, `static/js/profile-match-stats.js` — polling fallbacks (5s / 10–15s) и lifecycle cancellation; `static/js/team-utils.js` — унифицированный загрузчик логотипов.
...
- [x] Topic routing infra
  - [x] Добавить API: `subscribe(topic)`, `unsubscribe(topic)` для Socket.IO.
  - [x] Использовать room-based emits: `socketio.emit(event, data, room=topic)`.

- [x] Redis pub/sub per-topic invalidation
  - [x] Публикация событий `{event, topic, payload}` на канал `app:topic`.
  - [x] Другие инстансы должны получать эти события и форвардить их в локальный WebSocket менеджер.

- [x] Server-side batching / debounce
  - [x] Per-topic buffer 100–250ms, объединять поля (merge), версия v = max(vs).
  - [x] Если агрегатный размер превышает лимит или buffer содержит full → отправлять full snapshot.

- [x] Patch format + client apply/rollback
  - [x] Формат сообщения: `{type:'patch'|'full', topic, v, fields}`.
  - [x] Клиент при необходимости делает resync через `etag_json` (If-None-Match).
...
- [ ] Метрики и экспорт
  - [x] Собирать: `ws_messages_sent`, `ws_messages_batched`, `etag_hits`, `etag_misses`, `payload_build_ms`.
  - [ ] Экспортировать через `/health/perf` (admin-only, с `X-METRICS-KEY`).

### Edge cases и проверки

- [x] Out-of-order patches: клиент игнорирует старые версии (поле `v`).
- [ ] Large bursts: поставить hard cap на размер агрегированного patch; при превышении — отправлять full snapshot.
- [x] Offline clients: при reconnect клиент запрашивает full state через `etag_json` endpoint.
...
- [x] **Компактная новостная лента (не более 5 записей)**
  - [x] Простой блок новостей
    ...
    - ✅ Добавить CRUD в существующую админ-панель
