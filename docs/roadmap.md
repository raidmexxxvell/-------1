...
Подэтап 1.2b (следующий шаг) — Полноценные мастер-данные игроков:
- [ ] **API для игроков**: `POST /api/admin/players` (создание), `PUT /api/admin/players/<id>`, `DELETE /api/admin/players/<id>`
- [ ] **Схема players**: нормализованная таблица `players` (имя, фамилия, full_name, team_id(FK), position, number, birth_date, status, created_at, updated_at)
- [ ] **Миграция из TeamRoster**: одноразовый скрипт сопоставления строк по (team_name + нормализованное full_name) → создание записей в `players`
- [ ] **Расширение API команд**: включить счётчик активных игроков (для списков)
- [ ] **Админ-интерфейс**: вкладка «Игроки» (форма создания/редактирования, фильтр по команде, поиск)
- [ ] **Валидация**: уникальность (team_id, number) среди активных; position ∈ {GK, DEF, MID, FWD}
- [ ] **Критерий успеха**: >= 95% строк `team_roster` успешно мигрированы в `players`, поиск и фильтрация работают без деградации
   
	Промежуточно (выполнено): динамические таблицы статистики `team_stats_<team_id>` (ленивая инициализация + автообновление при завершении матча) для goals/assists/cards/matches.

Подэтап 1.2c — Интеграция статистики игроков (будет после нормализации):
- [ ] Агрегация: goals/assists/cards из `MatchEvent` / `PlayerStatistics`
- [ ] Расчёт snapshot `stats-table` опирается на нормализованные Player.id
- [ ] Обновление эндпоинта `/api/admin/teams/<id>/roster` → реальные данные вместо placeholder
- [ ] Критерий успеха: модалка состава показывает актуальные цифры, совпадающие с публичной статистикой
