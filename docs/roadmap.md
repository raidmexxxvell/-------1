...
- ✅ Клиент `static/js/profile-achievements.js`:
  ...
  - Отображаются «лестницы» целей из `all_targets` (например, «Цели: 7/30/120») и прогресс `value/next_target`.
...
- ✅ Этап 2: Серверные эндпоинты (admin-only)
  ...
  - ✅ POST `/api/admin/google/export-all` — экспорт в Sheets: пользователи, ставки, таблица (расширение набора данных — next).
...
- 🟨 Этап 4: Документация и метрики
  - 🟨 Обновить project.md (источники данных, права, переменные окружения для Sheets).
