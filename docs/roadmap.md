...
- [ ] **API для команд**: `POST/PUT/DELETE /api/admin/teams`
- [ ] **Админ-интерфейс**: Форма добавления/редактирования команд
- [x] **Автоматическое заполнение**: команды собираются по логотипам из `static/img/team-logos/`, город всегда "Обнинск", остальные поля опциональны
- [ ] **Критерий успеха**: Все команды из логотипов автоматически в БД, админ может управлять ими через UI
