...
### Анимации коэффициентов (новое)
- При обновлении коэффициента кнопка ставки получает класс `.updated` на 500–600 мс и проигрывает анимацию `odds-flash` (мягкая зелёная «подсветка» без смены фона).
- Анимация реализована через transform/box-shadow (GPU‑ускорено), учитывает предпочтения пользователя `prefers-reduced-motion` (браузер сам уменьшает интенсивность).

Класс: `.bet-btn.updated`
...
## 🗳️ Где показывается голосование
- Голосование доступно только во вкладке «Прогнозы» под логотипами команд соответствующих матчей.
- В расписании лиги голосование не отображается (исключено для упрощения UI и избежания дублей).
