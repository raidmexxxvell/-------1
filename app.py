...
if SECURITY_SYSTEM_AVAILABLE:
    ...
    try:
        ...
        try:
            ...
            app.config['WS_TOPIC_SCHEME'] = str(getattr(Config, 'WS_TOPIC_SCHEME', 'no_date'))
        except Exception:
            pass
    ...
    if SessionLocal is not None:
        ...
        try:
            ...
            if values:
                ...
                # Из реляционной таблицы, если снапшота нет
                try:
...
def get_stats_sheet():
    """Заглушка: Google Sheets удален из проекта"""
    raise RuntimeError('Google Sheets access disabled')

# [Removed] Google Sheets functions removed — Database is the source of truth.
...
def _build_schedule_payload_from_sheet():
    """Заглушка: Google Sheets удален из проекта"""
    return { 'updated_at': datetime.now(timezone.utc).isoformat(), 'tours': [] }
...
def _build_results_payload_from_sheet():
    """Заглушка: Google Sheets удален из проекта"""
    return { 'updated_at': datetime.now(timezone.utc).isoformat(), 'results': [] }
        ...
        try:
            ...
            try:
                ...
                if inv:
                    ...
                    scheme = str(app.config.get('WS_TOPIC_SCHEME','no_date'))
                    if scheme == 'with_date':
                        # Если нет даты — публикуем в бездатовый, чтобы не терять событие
                        topic = (f"match:{h}__{a}__{date_str}:details" if date_str else f"match:{h}__{a}__:details")
                    else:
                        # no_date (по умолчанию)
                        topic = f"match:{h}__{a}__:details"
                    ...
                    try:
                        ...
                        print(f"[API Stats Set] Ошибка publish_topic для {topic}: {pub_err}")
                else:
