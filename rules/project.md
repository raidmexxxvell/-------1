...
> Дата актуализации: 2025-09-03  
> Текущая версия `app.py`: ~9700 строк (рост из‑за сезонного deep reset, CRUD новостей, ETag, прогрева кэша, persistent roster).  
> Последние изменения: добавлена постоянная таблица `team_roster` + публичный эндпоинт lineups, real-time обновление и fallback логика.
...
- Автозагрузка новостей при старте SPA: `profile.js` теперь вызывает `loadNews()` на `DOMContentLoaded`, что устраняет проблему пустой ленты до переключения лиги.
- Клиентская часть достижений: в `static/js/profile-achievements.js` добавлена кнопка "Подробнее", блок с длинным описанием (`.achv-desc`) и тексты для дополнительных групп (`bigodds`, `markets`, `weeks`). Это убирает сообщения "Описание недоступно" для распространённых достижений.
- Стили: обновления в `static/css/style.css` — стили для `.achv-desc`, `.achv-desc-toggle` и прогресс-баров для единообразного отображения карточек достижений.
- Логирование: улучшены debug-сообщения в рендере достижений для диагностики целевых значений и прогресса.

Эти правки — небольшие, обратимые и повышают UX (особенно для новых пользователей при первом входе и для визуального представления достижений).
```
...
1. Вынести news / reset / snapshots / roster (lineups) из `app.py`.
2. Добавить bleach-sanitization контента новостей.
3. Инициализировать Alembic и первую ревизию (включая `team_roster`).
4. Настроить CI (lint + pytest stub).
5. Добавить OpenAPI генерацию.
