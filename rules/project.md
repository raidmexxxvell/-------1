...
- Подстроить TTL в `optimizations/multilevel_cache.py` для `schedule`/`results` с акцентом на снижение холодных загрузок.
- Добавить мягкий debounce (~250мс) на стороне сервера перед широковещательными WebSocket-патчами.
