...
(function () {
  ...
  function addToCart(item) {
    ...
    if (idx >= 0) {
      ...
      items.push({ id: item.id, name: item.name, price: Number(item.price) || 0, qty: 1 });
    }
    writeCart(items);
    renderCart();
  }
  ...
  function renderCart() {
    ...
    items.forEach(it => {
      ...
      minus.textContent = '−';
      minus.addEventListener('click', () => incQty(it.id, -1));
      ...
      plus.textContent = '+';
      plus.addEventListener('click', () => incQty(it.id, 1));
      qty.append(minus, input, plus);
      left.append(name, qty);

      const price = document.createElement('span');
      price.textContent = ((Number(it.price) || 0) * (Number(it.qty) || 1)).toLocaleString();
      const del = document.createElement('button');
      del.className = 'details-btn';
    ...
    totalEl.textContent = 'Итого: ' + total.toLocaleString() + ' кредитов';
    const checkout = document.createElement('button');
    checkout.className = 'details-btn';
  ...
  function initShop() {
    const store = document.getElementById('shop-pane-store');
    if (!store) {
      return;
    }
  ...
  async function placeOrder() {
    try {
      ...
      const r = await fetch('/api/shop/checkout', { method: 'POST', body: fd });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) {
        throw new Error(d?.error || 'Ошибка заказа');
      }
      writeCart([]);
      renderCart();
      try {
      ...
      try {
        window.Telegram?.WebApp?.showAlert?.('Не удалось оформить заказ');
      } catch (_) {}
    }
  }
  async function renderMyOrders() {
    const host = document.getElementById('shop-pane-myorders');
    if (!host) {
      return;
    }
    ...
    try {
      ...
      const fd = new FormData();
      fd.append('initData', tg?.initData || '');
      const r = await fetch('/api/shop/my-orders', { method: 'POST', body: fd });
  ...
  function initShopUI() {
    ...
    const panes = {
      store: document.getElementById('shop-pane-store'),
      cart: document.getElementById('shop-pane-cart'),
      myorders: document.getElementById('shop-pane-myorders'),
    };
    tabs.forEach(btn => {
