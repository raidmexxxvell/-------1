...
(function () {
  ...
  function initHomeAdsCarousel() {
    ...
    if (!slides || slides.length === 0) {
      slides = [
        {
          img: '/static/img/ligareklama.jpg',
          title: 'Здесь может быть ваша лига — нажми',
          action: 'BLB',
        },
          ...
          try {
            if (s.action === 'BLB') {
              ...
              if (window.selectBLBLeague) {
                ...
                if (window.setActiveLeague) {
                  try {
                    window.setActiveLeague('BLB');
                  } catch (_) {}
                }
                ...
                [
                  ...
                  if (el) {
                    el.style.display = id === 'tab-ufo' ? '' : 'none';
                  }
                });
                ...
                if (window.initBLBSubtabs) {
                  try {
                    window.initBLBSubtabs();
                  } catch (_) {}
                }
              }
            }
    ...
    apply();
    try {
      window.dispatchEvent(new CustomEvent('preload:ads-ready'));
    } catch (_) {}
    setTimeout(() => {
      try {
        window.dispatchEvent(new CustomEvent('main:ads-ready'));
      } catch (_) {}
    }, 0);
  }
  ...
  async function renderTopMatchOfWeek(overrideMatch) {
    try {
      ...
      let savedKey = null;
      try {
        savedKey = localStorage.getItem('feature-match-key') || null;
      } catch (_) {}
      ...
      const voteKey = (() => {
        try {
          const raw = m.date ? String(m.date) : m.datetime ? String(m.datetime) : '';
          const d = raw ? raw.slice(0, 10) : '';
          return `${(m.home || '').toLowerCase().trim()}__${(m.away || '').toLowerCase().trim()}__${d}`;
        } catch (_) {
          return `${(m.home || '').toLowerCase()}__${(m.away || '').toLowerCase()}__`;
        }
      })();
      const mkBtn = (code, text) => {
        ...
        b.addEventListener('click', async e => {
          ...
          try {
            ...
            const dkey = (m.date ? String(m.date) : m.datetime ? String(m.datetime) : '').slice(
              0,
              10
            );
            fd.append('date', dkey);
      ...
      try {
        segH.style.background = window.getTeamColor ? window.getTeamColor(m.home || '') : '#3b82f6';
        segA.style.background = window.getTeamColor ? window.getTeamColor(m.away || '') : '#3b82f6';
      ...
      try {
        ...
        card.addEventListener('click', e => {
          ...
          try {
            ...
            [elHome, elUfo, elPreds, elLead, elShop, elAdmin].forEach(x => {
              if (x) {
          ...
          const isEmptyRosters = (() => {
            try {
              const d = cached?.data;
              const h = Array.isArray(d?.rosters?.home) ? d.rosters.home : [];
              const a = Array.isArray(d?.rosters?.away) ? d.rosters.away : [];
              return h.length === 0 && a.length === 0;
            } catch (_) {
              return false;
            }
          })();
          if (cached && !isEmptyRosters && Date.now() - (cached.ts || 0) < FRESH_TTL) {
            go(cached);
      ...
      async function loadAgg(withInit) {
        try {
          ...
          const agg = await fetch(`/api/vote/match-aggregates?${params.toString()}`).then(r =>
            r.json()
          );
          const h = Number(agg?.home || 0),
            d = Number(agg?.draw || 0),
            a = Number(agg?.away || 0);
      ...
      loadAgg(true);
      try {
        window.dispatchEvent(new CustomEvent('preload:topmatch-ready'));
      } catch (_) {}
      setTimeout(() => {
        try {
          window.dispatchEvent(new CustomEvent('main:topmatch-ready'));
        } catch (_) {}
      }, 0);
    } catch (_) {}
  }
