...
class RealtimeUpdater {
    ...
    try {
      ...
      window.__wsDebug = function () {
        try {
          ...
          console.warn('[WS DEBUG] error', e);
          return { connected: false, topics: [], pending: [] };
        }
      };
    } catch (_) {}
  ...
  initSocket() {
    try {
      console.log(
        `[WS Инициализация] window.__WEBSOCKETS_ENABLED__: ${window.__WEBSOCKETS_ENABLED__}`
      );
      ...
      fetch(probeUrl, { method: 'GET', cache: 'no-store', redirect: 'manual' })
        .then(r => {
          ...
          try {
            window.__WEBSOCKETS_SOCKET = this.socket;
          } catch (_) {}
  ...
  setupEventHandlers() {
    ...
    this.socket.on('connect', () => {
      ...
      this.reconnectAttempts = 0;
      try {
        window.__WEBSOCKETS_CONNECTED = true;
      } catch (_) {}
      // No manual heartbeat: Socket.IO handles ping/pong internally
      try {
        window.RealtimeStore && window.RealtimeStore.set({ connected: true });
      } catch (_) {}
      ...
      try {
        if (this.topicEnabled) {
          ...
          if (glob && typeof glob.forEach === 'function') {
            glob.forEach(t => {
              try {
                this.pendingTopics.add(String(t));
              } catch (_) {}
            });
            try {
              glob.clear?.();
          ...
          this.pendingTopics.forEach(topic => {
            if (!this.subscribedTopics.has(topic)) {
              ...
              try {
                window.__WS_TOPIC_SUBSCRIBED?.add?.(topic);
              } catch (_) {}
              try {
                window.__WS_PENDING_SUBSCRIPTIONS?.delete?.(topic);
              } catch (_) {}
            }
          });
        }
    ...
    this.socket.on('disconnect', reason => {
      ...
      try {
        window.RealtimeStore && window.RealtimeStore.set({ connected: false });
      } catch (_) {}
    ...
    this.socket.on('match_finished', payload => {
      try {
        if (!payload || !payload.home || !payload.away) {
          return;
        }
        ...
        try {
          ...
          const btn = document.getElementById('md-finish-btn');
          if (btn) {
          ...
          if (teamPane && teamPane.style.display !== 'none') {
            ...
            if (openedTeam && (openedTeam === home || openedTeam === away)) {
              ...
              if (window.TeamPage && typeof window.TeamPage.openTeam === 'function') {
                ...
                setTimeout(() => {
                  try {
                    window.TeamPage.openTeam(openedTeam);
                  } catch (_) {}
                }, 50);
              }
            }
        ...
        try {
          if (typeof window.loadStatsViaStore === 'function') {
            window.loadStatsViaStore();
          } else if (typeof window.loadStatsTable === 'function') {
        ...
        if (!payload.results_block) {
          setTimeout(() => {
            try {
              this.triggerDataRefresh('schedule');
            } catch (_) {}
          }, 300);
        }
  ...
  handleTopicUpdate(payload) {
    try {
      ...
      console.log(
        '[Реалтайм] Обрабатываем обновление топика - сущность:',
        payload.entity,
        'топик:',
        topic,
        'причина:',
      ...
      try {
        if (payload.entity === 'match_stats' && payload.home && payload.away) {
          console.log(
            '[Реалтайм] Отправляем matchStatsRefresh для:',
            payload.home,
            'против',
            payload.away
          );
          const ev = new CustomEvent('matchStatsRefresh', {
            detail: { home: payload.home, away: payload.away },
          });
      ...
      try {
        if (
          (payload.entity === 'match_events' || payload.entity === 'match_events_removed') &&
          payload.home &&
          payload.away
        ) {
          const { home, away } = payload;
          ...
          const __ffDirectTopicEvents = (function () {
            try {
              return (localStorage.getItem('feature:ws_topic_update_direct_events') ?? '1') !== '0';
            } catch (_) {
              return true;
            }
          })();
          if (__ffDirectTopicEvents && payload.events && window.__MatchEventsRegistry) {
            try {
              ...
              if (matchDetailsPane && matchDetailsPane.style.display !== 'none') {
                const curHome =
                  matchDetailsPane.getAttribute('data-match-home') ||
                  matchDetailsPane.getAttribute('data-match-key') ||
                  '';
                const curAway = matchDetailsPane.getAttribute('data-match-away') || '';
                // Если это тот же матч — точечный ререндер, если есть API
                if (
                  (curHome && curAway && curHome === home && curAway === away) ||
                  (!curHome && !curAway)
                ) {
                  try {
                    if (typeof window.renderMatchRosters === 'function') {
                      window.renderMatchRosters(home, away, payload.events);
                    }
                  } catch (_) {}
                }
              }
      ...
      if (reason === 'full_reset') {
        ...
        try {
          ...
          for (let i = 0; i < localStorage.length; i++) {
            ...
            if (k.startsWith('voted:') || k.startsWith('voteAgg:')) {
              toDel.push(k);
            }
          }
          toDel.forEach(k => {
          ...
          document.querySelectorAll('.vote-inline').forEach(wrap => {
            try {
              ...
              if (window.__VoteAgg && typeof window.__VoteAgg.fetchAgg === 'function') {
                window.__VoteAgg
                  .fetchAgg(home, away, date)
                  .then(agg => {
                    try {
                      if (typeof wrap.__applyAgg === 'function') {
                        wrap.__applyAgg(agg);
                      }
                    } catch (_) {}
                  })
    ...
    try {
      if (entity === 'match') {
        ...
        if (id && id.home && id.away) {
          ...
          const other = { ...fields };
          delete other.score_home;
          delete other.score_away;
      ...
      if (entity === 'odds') {
        ...
        const incomingV =
          fields && fields.odds_version != null ? Number(fields.odds_version) : null;
        if (incomingV != null) {
  ...
  _setOddsVersion(home, away, v) {
    try {
      this.oddsVersions.set(this._ovKey(home, away), Number(v) || 0);
    } catch (_) {}
  }
  ...
  scheduleReconnect() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.warn(
        `🔌 Max reconnect attempts (${this.maxReconnectAttempts}) reached, попробуем полную реинициализацию`
      );
  ...
  scheduleInitRetry() {
    ...
    console.log(
      `🔄 Повторная инициализация WebSocket через ${delay / 1000}с (попытка ${this.initAttempts}/${this.maxInitAttempts})`
    );
  ...
  updateMatchScore(home, away, data) {
    ...
    try {
      if (!window.__scoreUpdates) {
        window.__scoreUpdates = [];
      }
    } catch (_) {}
    const ts = Date.now();
        ...
        if (existingKey) {
          ...
          if (entry?.score) {
            prevStoreScore = {
              ...
              txt: `${entry.score.home} : ${entry.score.away}`,
            };
          }
        }
      }
    ...
    try {
      if (window.MatchesStoreAPI) {
        ...
        const k = window.MatchesStoreAPI.addOrMergeMatch
          ? window.MatchesStoreAPI.addOrMergeMatch({ home, away, score: { home: sh, away: sa } })
          : (function () {
              let tmp = window.MatchesStoreAPI.findMatchByTeams(home, away);
              if (!tmp) {
                window.MatchesStoreAPI.updateMatch(
                  home.toLowerCase().trim() + '__' + away.toLowerCase().trim(),
                  { home, away, score_home: sh, score_away: sa }
                );
                tmp = window.MatchesStoreAPI.findMatchByTeams(home, away);
              }
              return tmp;
            })();
        if (k) {
          ...
          try {
            ...
            const changed =
              !prevStoreScore || prevStoreScore.home !== sh || prevStoreScore.away !== sa;
            const rec = {
              ts,
              source: 'realtime-updates.updateMatchScore',
              home,
              away,
              newScore: { home: sh, away: sa },
              prev: prevStoreScore,
              matchKey: k,
              changed,
            };
            window.__scoreUpdates.push(rec);
            if (window.__scoreUpdates.length > 400) {
              window.__scoreUpdates.splice(0, window.__scoreUpdates.length - 400);
            }
        ...
        matchElements.forEach(el => {
          const scoreElement = el.querySelector('.match-score') || el.querySelector('.score');
          if (scoreElement && scoreElement.textContent !== txt) {
        ...
        try {
          const rec = {
            ts,
            source: 'realtime-updates.updateMatchScore:fallback',
            home,
            away,
            newScore: { home: sh, away: sa },
            prev: prevStoreScore,
          ...
          if (window.localStorage?.getItem('debug:score_log') === '1') {
            console.log('%c[ScoreUpdate]', 'color:#f59e0b', rec);
          }
        } catch (_) {}
      }
    } catch (e) {
  ...
  subscribeTopic(topic) {
    try {
      ...
      try {
        if (window.__WS_TOPIC_SUBSCRIBED?.has?.(topic)) {
          return;
        }
      ...
      if (this.socket && this.isConnected && !this.subscribedTopics.has(topic)) {
        ...
        this.subscribedTopics.add(topic);
        console.log(
          '[Реалтайм] Подписались на топик:',
          topic,
          'Всего подписок:',
          this.subscribedTopics.size
        );
        try {
          window.__WS_TOPIC_SUBSCRIBED?.add?.(topic);
        } catch (_) {}
        ...
        console.log(
          '[Реалтайм] Не можем подписаться сейчас - сокет:',
          !!this.socket,
          'подключен:',
          this.isConnected,
          'уже подписан:',
  ...
  unsubscribeTopic(topic) {
    try {
      ...
      try {
        window.__PENDING_WS_TOPICS__?.delete?.(topic);
      } catch (_) {}
      try {
        window.__WS_TOPIC_SUBSCRIBED?.delete?.(topic);
      } catch (_) {}
      try {
        window.__WS_PENDING_SUBSCRIPTIONS?.delete?.(topic);
      } catch (_) {}
  ...
  hasTopic(topic) {
    try {
      ...
      return (
        (this.subscribedTopics && this.subscribedTopics.has(topic)) ||
        (this.pendingTopics && this.pendingTopics.has(topic)) ||
        (window.__PENDING_WS_TOPICS__ &&
          typeof window.__PENDING_WS_TOPICS__.has === 'function' &&
          window.__PENDING_WS_TOPICS__.has(topic))
      );
    } catch (_) {
      return false;
    }
