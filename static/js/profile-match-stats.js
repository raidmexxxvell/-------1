...
(function(){
  function render(host, match){
    ...
    const applyUpdate=(d)=>{
      metrics.forEach(mt=>{ const built=rows.get(mt.key); if(!built) return; const vals= d && Array.isArray(d[mt.key])? d[mt.key]:[0,0]; const l=Number(vals[0])||0; const r=Number(vals[1])||0; try {
    ...
    const loop = async ()=>{
      ...
      try {
        ...
        const sig = (()=>{ try { return metrics.map(m=>{
          const v=Array.isArray(data[m.key])?data[m.key]:[0,0]; return `${v[0]||0}-${v[1]||0}`; }).join('|'); } catch(_){ return null; } })();
