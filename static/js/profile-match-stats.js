...
(function () {
  function render(host, match) {
    try {
      if (host.__statsPollCancel) {
        host.__statsPollCancel();
      }
    } catch (_) {}
    ...
    const updateStatBar = (row, leftVal, rightVal) => {
      try {
        ...
        if (leftFill && rightFill) {
          leftFill.style.transition = 'width .3s ease';
          rightFill.style.transition = 'width .3s ease';
    ...
    const buildOnce = d => {
      ...
      const bar = (l, r) => {
        ...
        const leftValEl = document.createElement('div');
        leftValEl.className = 'stat-val';
        leftValEl.textContent = String(leftVal);
        leftSide.appendChild(leftValEl);
        ...
        rightFill.style.width = rp + '%';
        try {
          leftFill.style.backgroundColor = getTeamColor(match.home || '');
          rightFill.style.backgroundColor = getTeamColor(match.away || '');
        } catch (_) {}
        mid.append(leftFill, rightFill);
        const rightSide = document.createElement('div');
        rightSide.className = 'stat-side stat-right';
        const rightValEl = document.createElement('div');
        rightValEl.className = 'stat-val';
        rightValEl.textContent = String(rightVal);
        rightSide.appendChild(rightValEl);
        row.append(leftSide, mid, rightSide);
        return { row, leftVal: leftValEl, rightVal: rightValEl };
      };
      metrics.forEach(mt => {
        ...
        try {
          ...
          const isAdmin =
            window.AdminUtils?.isCurrentUserAdmin?.() ||
            (() => {
          ...
          if (isAdmin) {
            ...
            const createAdminBtn =
              window.AdminUtils?.createAdminButton ||
              (text => {
            ...
            const lh = createAdminBtn('−'),
              lplus = createAdminBtn('+'),
              rh = createAdminBtn('−'),
              rplus = createAdminBtn('+');
            const leftBox = document.createElement('div');
            leftBox.className = 'admin-inc';
            const rightBox = document.createElement('div');
            rightBox.className = 'admin-inc';
            const base = mt.key;

            const post = (lv, rv) => {
              // Используем новые утилиты для FormData
              const fd =
                window.AdminUtils?.createAdminFormData?.({
                  home: match.home || '',
                  away: match.away || '',
            ...
            const anim = (el, from, to) => {
              if (window.CounterAnimation) {
                window.CounterAnimation.animate(el, from, to, 200);
              } else {
                el.textContent = String(to);
              }
              el.classList.add('stat-update-animation');
              setTimeout(() => el.classList.remove('stat-update-animation'), 300);
            };
            const getVals = () => {
              const l = parseInt(built.leftVal.textContent, 10) || 0;
              const r = parseInt(built.rightVal.textContent, 10) || 0;
              return [l, r];
            };
            lh.addEventListener('click', () => {
              const [l, r] = getVals();
              const nv = Math.max(0, l - 1);
              anim(built.leftVal, l, nv);
              updateStatBar(built.row, nv, r);
              post(nv, r);
            });
            ...
            rh.addEventListener('click', () => {
              const [l, r] = getVals();
              const nv = Math.max(0, r - 1);
              anim(built.rightVal, r, nv);
              updateStatBar(built.row, l, nv);
              post(l, nv);
            });
            ...
            leftBox.append(lh, lplus);
            rightBox.append(rh, rplus);
            const leftSide = built.row.querySelector('.stat-left');
            const rightSide = built.row.querySelector('.stat-right');
            if (leftSide) {
              leftSide.insertBefore(leftBox, leftSide.firstChild);
            }
            if (rightSide) {
        ...
        rowWrap.append(title, built.row);
        wrap.appendChild(rowWrap);
        rows.set(mt.key, built);
      });
      host.innerHTML = '';
    ...
    const applyUpdate = d => {
      metrics.forEach(mt => {
        ...
        try {
          const curL = parseInt(built.leftVal.textContent, 10) || 0;
          const curR = parseInt(built.rightVal.textContent, 10) || 0;
          if (curL !== l) {
            if (window.CounterAnimation) {
              window.CounterAnimation.animate(built.leftVal, curL, l, 200);
            } else {
              built.leftVal.textContent = String(l);
            }
          }
          if (curR !== r) {
            if (window.CounterAnimation) {
              window.CounterAnimation.animate(built.rightVal, curR, r, 200);
            } else {
              built.rightVal.textContent = String(r);
            }
          }
    ...
    const wsActive = () => {
      try {
        return !!(
          ...
          window.realtimeUpdater.hasTopic &&
          window.realtimeUpdater.getTopicEnabled() &&
          window.realtimeUpdater.hasTopic(wsTopic)
        );
      } catch (_) {
        return false;
      }
    };
    let wsRefreshHandler = null;
    let watch = null;
    host.__statsPollCancel = () => {
      ...
      try {
        if (wsRefreshHandler) {
          document.removeEventListener('matchStatsRefresh', wsRefreshHandler);
        }
      } catch (_) {}
    };
    const schedule = () => {
    ...
    const loop = async () => {
      ...
      try {
        ...
        if (sig && state.sig && sig === state.sig) {
          return;
        }
        if (rows.size === 0) {
          buildOnce(data);
      ...
      try {
        ...
        fetch(url, { headers: state.etag ? { 'If-None-Match': state.etag } : {} })
          .then(async r => {
            ...
            const d = r.status === 304 ? null : await r.json().catch(() => null);
            if (d) {
              if (rows.size === 0) {
                buildOnce(d);
              } else {
              ...
              state.sig = metrics
                .map(m => {
                  const v = Array.isArray(d[m.key]) ? d[m.key] : [0, 0];
                  const l = v[0] === null || v[0] === undefined ? 0 : Number(v[0]) || 0;
                  const r = v[1] === null || v[1] === undefined ? 0 : Number(v[1]) || 0;
                  return `${l}-${r}`;
                })
