...
(function () {
  function openMatchScreen(match, details) {
    try {
      window.__CURRENT_MATCH_KEY__ = `${(match?.home || '').toLowerCase().trim()}__${(match?.away || '').toLowerCase().trim()}__${(match?.datetime || match?.date || '').toString().slice(0, 10)}`;
    } catch (_) {}
    ...
    try {
      ...
      [tablePane, statsPaneLeague, schedulePane, resultsPane].forEach(p => {
        if (p) {
    ...
    try {
      document.getElementById('ufo-subtabs').style.display = 'none';
    } catch (_) {}
    ...
    try {
      mdPane.setAttribute('data-match-home', match.home || '');
      mdPane.setAttribute('data-match-away', match.away || '');
    } catch (_) {}
    const setLogo = (imgEl, name) => {
      try {
        (window.setTeamLogo || window.TeamUtils?.setTeamLogo || function () {})(imgEl, name || '');
      } catch (_) {}
    };
    hName.setAttribute('data-team-name', match.home || '');
    aName.setAttribute('data-team-name', match.away || '');
    try {
      hLogo?.setAttribute('data-team-name', match.home || '');
      aLogo?.setAttribute('data-team-name', match.away || '');
    } catch (_) {}
    hName.textContent = window.withTeamCount
      ? window.withTeamCount(match.home || '')
      : match.home || '';
    aName.textContent = window.withTeamCount
      ? window.withTeamCount(match.away || '')
      : match.away || '';
    ...
    try {
      ...
      if (!hasDigits) {
        ...
        if (!eff) {
          try {
            const keyCache =
              (window.MatchUtils ? window.MatchUtils.matchKey(match) : null) ||
              window.__CURRENT_MATCH_KEY__;
            const st2 =
              window.MatchState && window.MatchState.get ? window.MatchState.get(keyCache) : null;
            if (st2 && st2.score) {
              eff = st2.score;
            }
          } catch (_) {}
      ...
      /* избегаем принудительного плейсхолдера чтобы не вызывать мерцание */
    }
    try {
    ...
    try {
      ...
      if (storeApi) {
        ...
        let scoreHomeSeed = null;
        let scoreAwaySeed = null;
        if (
          details &&
        ...
        if (
          scoreHomeSeed != null &&
          scoreAwaySeed != null &&
          Number.isFinite(scoreHomeSeed) &&
          Number.isFinite(scoreAwaySeed)
        ) {
        ...
        if (matchKey && details) {
          ...
          if (Array.isArray(details?.events)) {
            patch.events = details.events;
          }
          if (
            details?.rosters &&
            (Array.isArray(details.rosters.home) || Array.isArray(details.rosters.away))
          ) {
            patch.rosters = details.rosters;
          }
          if (details?.stats && typeof details.stats === 'object') {
            patch.stats = details.stats;
          }
          if (Object.keys(patch).length) {
            try {
              storeApi.updateMatch(
                matchKey,
                Object.assign({ home: payload.home, away: payload.away, date: payload.date }, patch)
              );
            } catch (_) {}
          }
        }
      ...
      console.warn('[MatchAdvanced] Не удалось прогреть MatchesStore:', e);
    }
    ...
    try {
      if (window.__WS_TOPIC_SUBS__ && window.realtimeUpdater) {
        ...
        try {
          mdPane.__wsTopics = __topics.slice();
        } catch (_) {}
        console.log(
          '[WS Матч] Единственный топик для подписки:',
          __topic,
        ...
        setTimeout(() => {
          try {
            ...
            __topics.forEach(topic => {
              ...
              const lastTs = (function () {
                try {
                  return window.__WS_LAST_SUBSCRIBE_TS?.get?.(topic) || 0;
                } catch (_) {
                  return 0;
                }
              })();
              ...
              const already = (function () {
                try {
                  return !!window.__WS_TOPIC_SUBSCRIBED?.has?.(topic);
                } catch (_) {
                  return false;
                }
              })();
              if (already || recently) {
                console.log(
                  '[WS Матч] Пропускаем повторную подписку:',
                  topic,
                  'already=',
                  already,
                  'recently=',
              ...
              console.log('[WS Матч] Попытка подписки на топик:', topic);
              ru.subscribeTopic(topic);
              try {
                window.__WS_LAST_SUBSCRIBE_TS?.set?.(topic, Date.now());
              } catch (_) {}
              console.log('[WS Матч] Подписка выполнена для:', topic);
            });
          } catch (e) {
        ...
        console.warn(
          '[WS Матч] Подписка на топики отключена. WS_TOPIC_SUBS:',
          !!window.__WS_TOPIC_SUBS__,
          'realtimeUpdater:',
    ...
    try {
      const mkKey = o => {
        const h = (o?.home || '').toLowerCase().trim();
        const a = (o?.away || '').toLowerCase().trim();
        const raw = o?.date ? String(o.date) : o?.datetime ? String(o.datetime) : '';
        const d = raw ? raw.slice(0, 10) : '';
        return `${h}__${a}__${d}`;
      };
      mdPane.setAttribute('data-match-key', mkKey(match));
    ...
    mdPane
      .querySelectorAll('.modal-subtabs .subtab-item')
      .forEach(el => el.classList.remove('active'));
    try {
      const tabHome = subtabs?.querySelector('[data-mdtab="home"]');
      const tabAway = subtabs?.querySelector('[data-mdtab="away"]');
      if (tabHome) {
        tabHome.textContent = match.home || 'Команда 1';
      }
      if (tabAway) {
        tabAway.textContent = match.away || 'Команда 2';
      }
    } catch (_) {}
    ...
    try {
      ...
      if (present.has(thisKey) && isAdmin) {
        if (!existed) {
          const sp = document.createElement('div');
          sp.className = 'subtab-item';
          sp.setAttribute('data-mdtab', 'specials');
          sp.textContent = 'Спецсобытия';
    ...
    if (!subtabs.querySelector('[data-mdtab="stats"]')) {
      ...
      st.className = 'subtab-item';
      st.setAttribute('data-mdtab', 'stats');
      st.textContent = 'Статистика';
    ...
    try {
      if (window.MatchRostersEvents?.render) {
        window.MatchRostersEvents.render(match, details, mdPane, { homePane, awayPane });
      }
    } catch (_) {}
    ...
    try {
      const hasRosters = (() => {
        try {
          const h = Array.isArray(details?.rosters?.home) ? details.rosters.home : [];
          const a = Array.isArray(details?.rosters?.away) ? details.rosters.away : [];
          return h.length + a.length > 0;
        } catch (_) {
          return false;
        }
      })();
    ...
    try {
      if (window.MatchLiveScore?.setup) {
        liveScoreCtx = window.MatchLiveScore.setup(match, { scoreEl: score, dtEl: dt, mdPane });
      }
    } catch (_) {}
    // preload stats & specials (modular)
    try {
      if (window.MatchStats?.render) {
        window.MatchStats.render(statsPane, match);
      }
    } catch (e) {
      console.error('preload stats err', e);
    }
    try {
      if (window.MatchSpecials?.render) {
        window.MatchSpecials.render(specialsPane, match);
      }
    } catch (e) {
      console.error('preload specials err', e);
    }
    ...
    try {
      ...
      const shouldPoll =
        !wsEnabled || !wsTopicEnabled || localStorage.getItem('debug:force_match_polling') === '1';
      ...
      if (shouldPoll && window.fetchMatchDetails) {
        ...
        try {
          if (mdPane.__detailsPollCancel) {
            mdPane.__detailsPollCancel();
          }
        } catch (_) {}
        let cancelled = false;
        let busy = false;
        ...
        const loop = async () => {
          if (cancelled || mdPane.style.display === 'none') {
            return;
          }
          ...
          try {
            ...
            if (store && ver && ver !== lastVersion) {
              console.log(
                '[Поллинг Матча] Обнаружена новая версия:',
                ver,
                'предыдущая:',
                lastVersion
              );
              ...
              try {
                ...
                if (!justChanged) {
                  if (window.MatchRostersEvents?.render) {
                    window.MatchRostersEvents.render(match, store.data, mdPane, {
                      homePane,
                      awayPane,
                    });
                  }
    ...
    mdPane.querySelectorAll('.modal-subtabs .subtab-item').forEach(btn => {
      btn.onclick = () => {
        ...
        if (key !== 'stream') {
          ...
          try {
            if (window.MatchStream?.deactivate) {
              window.MatchStream.deactivate(streamPane);
            }
          } catch (_) {}
        }
        if (key === 'home') {
          ...
          homePane.style.display = 'none';
          awayPane.style.display = 'none';
          specialsPane.style.display = '';
          try {
            if (window.MatchSpecials?.render) {
              window.MatchSpecials.render(specialsPane, match);
            }
          } catch (e) {
            console.error('specials render err', e);
          }
          statsPane.style.display = 'none';
          if (streamPane) {
            streamPane.style.display = 'none';
          ...
          if (window.MatchStream?.setup) {
            try {
              streamPane = window.MatchStream.setup(mdPane, subtabs, match) || streamPane;
            } catch (_) {}
          }
          if (window.Streams?.setupMatchStream) {
            try {
              const hadTab = !!subtabs.querySelector('[data-mdtab="stream"]');
              streamPane = window.Streams.setupMatchStream(mdPane, subtabs, match) || streamPane;
              if (!hadTab) {
                /* tab now created */
              }
          ...
          if (streamPane) {
            try {
              if (window.MatchStream && typeof window.MatchStream.activate === 'function') {
                ...
                window.Streams &&
                typeof window.Streams.onStreamTabActivated === 'function'
              ) {
          ...
          try {
            ...
            console.error('stats render err', e);
          }
        }
      };
    });
    ...
    try {
      ...
      if (isAdmin && topbar) {
        ...
        refreshServerStatus();
        if (mdPane.__finishBtnSrvTimer) {
          try {
            clearInterval(mdPane.__finishBtnSrvTimer);
          } catch (_) {}
        }
        mdPane.__finishBtnSrvTimer = setInterval(refreshServerStatus, 60000);
        const applyVisibility = () => {
          const show = !finStore[mKey] && (isLiveNow(match) || serverLive);
    ...
    let adminCtx = null;
    try {
      if (window.MatchAdmin?.setup) {
        adminCtx = window.MatchAdmin.setup(match, { mdPane });
      }
    } catch (_) {}
    ...
    if (back) {
      back.onclick = () => {
        try {
          if (window.__WS_TOPIC_SUBS__ && window.realtimeUpdater) {
            const topicsToClean =
              Array.isArray(__topics) && __topics.length ? __topics : __topic ? [__topic] : [];
            topicsToClean.forEach(topic => {
              try {
                window.realtimeUpdater.unsubscribeTopic(topic);
              } catch (_) {}
            });
          }
        } catch (_) {}
        try {
          if (mdPane.__onDetailsUpdate) {
            document.removeEventListener('matchDetailsUpdate', mdPane.__onDetailsUpdate);
          }
        } catch (_) {}
        try {
          if (mdPane.__detailsPollCancel) {
        ...
        try {
          const statsHost = document.getElementById('md-pane-stats');
          if (statsHost && statsHost.__statsPollCancel) {
            statsHost.__statsPollCancel();
          }
        } catch (_) {}
        homePane.innerHTML = '';
        awayPane.innerHTML = '';
        ...
        try {
          if (liveScoreCtx) {
            liveScoreCtx.cleanup();
          }
        } catch (_) {}
        try {
          if (window.Streams?.resetOnLeave) {
            window.Streams.resetOnLeave(mdPane);
          }
        } catch (_) {}
        try {
          const spLeak = document.getElementById('md-pane-stream');
          if (spLeak) {
            spLeak.classList.remove('fs-mode');
          }
        } catch (_) {}
        try {
          document.body.classList.remove('allow-landscape');
        ...
        try {
          document.getElementById('ufo-subtabs').style.display = '';
        } catch (_) {}
      };
    }
  }
  ...
  try {
    window.openMatchScreen = openMatchScreen;
  } catch (_) {}
})();
