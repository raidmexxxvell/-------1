...
(function () {
  try {
    ...
    function readBest() {
      try {
        return JSON.parse(localStorage.getItem(LS_BEST_KEY) || '{}');
      } catch (_) {
        return {};
      }
    }
    function hasBestBaseline() {
      try {
    ...
    function writeBest(map) {
      try {
        localStorage.setItem(LS_BEST_KEY, JSON.stringify(map || {}));
      } catch (_) {}
    }
    function readPending() {
      try {
        return JSON.parse(localStorage.getItem(LS_PENDING_KEY) || 'null');
      } catch (_) {
        return null;
      }
    }
    ...
    function getActiveTab() {
      try {
        ...
        if (window.UIStore?.get) {
          return window.UIStore.get().activeTab || '';
        }
    ...
    function addProfileDot() {
      try {
        const navItem = document.querySelector('.nav-item[data-tab="profile"]');
        if (!navItem) {
          return;
        }
        ...
        if (!badge) {
          ...
          badge.className = 'nav-badge nav-badge--dot';
    ...
    function removeProfileDot() {
      try {
        document.querySelector('.nav-item[data-tab="profile"] .nav-badge')?.remove();
      } catch (_) {}
    }
    ...
    function summarizeUnlocks(achievements) {
      ...
      let totalXp = 0,
        totalCr = 0;
      unlocked.forEach(u => {
        const r = REWARDS_BY_TIER[u.tier] || { xp: 0, credits: 0 };
        totalXp += r.xp || 0;
        totalCr += r.credits || 0;
      });
    ...
    function buildIconCandidates(item) {
      ...
      if (item.icon_url) {
        candidates.push(item.icon_url);
      }
      if (key) {
        candidates.push(`${base}${slugify(key)}-${state}.png`);
      }
      if (key && item.icon) {
        candidates.push(`${base}${slugify(key)}-${slugify(item.icon)}.png`);
      }
      candidates.push(`${base}${state}.png`);
      candidates.push(`${base}placeholder.png`);
      // svg варианты
      candidates.slice().forEach(p => {
        if (/\.png$/i.test(p)) {
          const s = p.replace(/\.png$/i, '.svg');
    ...
    async function showPending(custom) {
      ...
      try {
        ...
        if (pending.items?.length > 1) {
          ...
          subtitle = pending.items
            .map(it => `${it.name || it.group} (${it.tier})`)
            .slice(0, 3)
            .join(', ');
      ...
      if (window.RewardAnimation) {
        try {
          window.RewardAnimation.show(document.body, pending.xp || 0, pending.credits || 0, {
            title,
            subtitle,
            imageUrl: img,
          }).catch(() => {});
      ...
      try {
        if (window.ProfileCheckin?.animateStats) {
          window.ProfileCheckin.animateStats(pending.xp || 0, pending.credits || 0, baseUser);
        }
      } catch (_) {}
    ...
    console.warn('achievements-notify init failed', e);
  }
})();
