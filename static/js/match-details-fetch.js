...
(function () {
  ...
  function cacheKey(home, away) {
    return `md:${(home || '').toLowerCase()}::${(away || '').toLowerCase()}`;
  }
  ...
  async function legacyFetch(paramsStr, cached) {
    const r = await fetch(`/api/match-details?${paramsStr}`, {
  ...
  async function fetchMatchDetails(opts) {
    ...
    if (!home || !away) {
      throw new Error('home & away required');
    }
    ...
    if (!window.fetchEtag) {
      ...
      try {
        return await legacyFetch(params.toString(), cached);
      } catch (e) {
        if (cached) {
          return cached;
        }
    ...
    const res = await window
      .fetchEtag(`/api/match-details?${params.toString()}`, {
      ...
      .catch(err => {
        if (cached) {
          return { data: cached.data, etag: cached.version, updated: false };
        }
        throw err;
      });
    ...
    const incomingRosters = (() => {
      try {
        const h = Array.isArray(data?.rosters?.home) ? data.rosters.home : [];
        const a = Array.isArray(data?.rosters?.away) ? data.rosters.away : [];
    ...
    const cachedRosters = (() => {
      try {
        const h = Array.isArray(cached?.data?.rosters?.home) ? cached.data.rosters.home : [];
        const a = Array.isArray(cached?.data?.rosters?.away) ? cached.data.rosters.away : [];
        return h.length + a.length;
      } catch (_) {
        return 0;
      }
