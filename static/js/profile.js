...
(() => {
  try {
    ...
    if (window.__PROFILE_FETCH_LIMITED__) {
      return;
    }
    ...
    const cfg = Object.assign(
      { tokensPerSec: 20, bucketCapacity: 20, maxConcurrent: 6 },
    ...
    setInterval(() => {
      tokens = Math.min(cfg.bucketCapacity, tokens + cfg.tokensPerSec);
      schedule();
    }, 1000);
  ...
  function setupEventListeners() {
    ...
    if (elements.editName) {
      elements.editName.setAttribute('data-throttle', '1500');
    }
    ...
    navItems.forEach(item => {
      ...
      if (tab === 'ufo') {
        item.setAttribute('data-throttle', '0');
      } else {
      ...
      item.addEventListener('click', () => {
        ...
        try {
          ...
          if (mdPane && mdPane.style.display !== 'none') {
            ...
            try {
              if (window.Streams && typeof window.Streams.resetOnLeave === 'function') {
                window.Streams.resetOnLeave(mdPane);
              }
            } catch (_) {}
            ...
            const st = document.getElementById('ufo-subtabs');
            if (st) {
              st.style.display = '';
            }
          }
        ...
        try {
          ...
          if (cont) {
            cont.classList.remove('profile-hide-top');
          }
          const ph = document.querySelector('.profile-header');
          if (ph) {
            ph.classList.remove('profile-centered');
          }
        ...
        [home, prof, ufo, preds, lead, shop, admin].forEach(el => {
          if (el) {
        ...
        if (tab === 'profile' && prof) {
          ...
          try {
            ...
            const cont = document.querySelector('.container');
            if (cont) {
              cont.classList.add('profile-hide-top');
            }
            const ph = document.querySelector('.profile-header');
            if (ph) {
              ph.classList.add('profile-centered');
            }
        ...
        if (tab === 'ufo' && ufo) {
          ...
          try {
            ...
            if (act === 'BLB') {
              selectBLBLeague(false);
            } else {
              selectUFOLeague(true, false);
            }
        ...
        if (tab === 'predictions' && preds) {
          ...
          if (window.__ACTIVE_LEAGUE__ === 'BLB') {
            ...
            if (host) {
              host.textContent = 'Скоро...';
            }
            const myb = document.getElementById('my-bets');
            if (myb) {
              myb.textContent = 'Скоро...';
            }
          } else {
            try {
              window.loadBetTours?.();
        ...
        if (tab === 'leaderboard' && lead) {
          lead.style.display = '';
          ensureLeaderboardInit();
        }
        if (tab === 'shop' && shop) {
          shop.style.display = '';
          try {
            window.Shop?.initShopUI?.();
          } catch (_) {}
        }
        if (tab === 'admin' && admin) {
          admin.style.display = '';
          try {
            window.Admin?.ensureAdminInit?.();
          } catch (_) {}
        }
    ...
    subtabItems.forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        try {
          console.log('[ufo-subtab-click]', btn.getAttribute('data-subtab'));
        } catch (_) {}
        ...
        Object.values(subtabMap).forEach(el => {
          if (el) {
            el.style.display = 'none';
          }
        });
        ...
        refreshBtn.addEventListener('click', async () => {
          ...
          try {
            await Promise.allSettled([
              Promise.resolve(loadLeagueTable()),
              Promise.resolve(loadStatsTable()),
            ]);
          } catch (_) {}
          try {
            localStorage.removeItem('schedule:tours');
            localStorage.removeItem('results:list');
          } catch (_) {}
          try {
            loadSchedule();
          } catch (_) {}
  ...
  function initHomeAdsCarousel() {
    ...
    if (!slides || slides.length === 0) {
      slides = [
        { img: '/static/img/ligareklama.jpg', title: 'Нажми', action: 'BLB' },
      ...
      slide.addEventListener('click', () => {
        try {
          ...
          if (act === 'BLB') {
            ...
            [elHome, elUfo, elPreds, elLead, elShop, elAdmin].forEach(el => {
              if (el) {
                el.style.display = 'none';
              }
            });
  ...
  async function loadNews() {
    ...
    try {
      if (document.getElementById('news-track')) {
        return;
      }
    } catch (_) {}
  ...
  try {
    window.getScheduleCached = getScheduleCached;
  } catch (_) {}
    ...
    try {
      ...
      if (overrideMatch && typeof overrideMatch === 'object') {
        ...
        const cached = (function () {
          try {
            return JSON.parse(localStorage.getItem('schedule:tours') || 'null');
          } catch (_) {
            return null;
          }
        })();
        ...
        if (sched && typeof sched === 'object' && sched.match_of_week) {
          ...
          try {
            const r = await window.fetchEtag('/api/schedule', {
              cacheKey: 'profile:topmatch',
              extract: j => j?.data || j,
            });
            ...
            try {
              localStorage.setItem(
                'schedule:tours',
                JSON.stringify({ data, version: r.etag || null, ts: Date.now() })
              );
            } catch (_) {}
          } catch (_) {
            m = null;
      ...
      host.innerHTML = '';
      if (!m) {
        host.innerHTML = '<div style="color: var(--gray);">Скоро анонс матча недели</div>';
        return;
      }
      // Карточка
      const card = document.createElement('div');
      card.className = 'match-card home-feature';
      ...
      const loadLogo = (imgEl, teamName) => {
        try {
          (window.setTeamLogo || window.TeamUtils?.setTeamLogo || function () {})(
            imgEl,
            teamName || ''
          );
        } catch (_) {}
      ...
      try {
        window.ScoreDOMAdapter?.attach &&
          window.ScoreDOMAdapter.attach(scoreEl, { home: m.home, away: m.away });
      } catch (_) {}
      // Горизонтальная полоса голосования — через унифицированный helper
      const wrap = window.VoteInline?.create?.({
        home: m.home,
        away: m.away,
      ...
      const toursCache = (() => {
        try {
          return JSON.parse(localStorage.getItem('betting:tours') || 'null');
        } catch (_) {
          return null;
        }
      })();
      ...
      try {
        ...
        tours.forEach(t => (t.matches || []).forEach(x => tourMatches.add(mkKey(x))));
      } catch (_) {}
      if (tourMatches.has(mkKey(m)) && wrap) {
      ...
      try {
        ...
        card.addEventListener('click', e => {
          ...
          if (window.fetchMatchDetails) {
            window
              .fetchMatchDetails({ home: m.home, away: m.away })
              .then(go)
              .catch(() => {});
          } else {
      ...
      const goPred = document.createElement('button');
      goPred.className = 'details-btn';
      goPred.textContent = 'Сделать прогноз';
      goPred.addEventListener('click', e => {
        try {
          e.stopPropagation();
        } catch (_) {}
        try {
          document.querySelector('.nav-item[data-tab="predictions"]').click();
        } catch (_) {}
      });
      footer.appendChild(goPred);
  ...
  try {
    window.renderTopMatchOfWeek = renderTopMatchOfWeek;
  } catch (_) {}
  ...
  function loadLeagueTable() {
    ...
    try {
      if (
        localStorage.getItem('feature:league_ui_store') === '1' &&
        typeof window.loadLeagueTableViaStore === 'function'
      ) {
        window
  ...
  function setUpdatedLabelSafely(labelEl, newIso) {
    try {
      window.League?.setUpdatedLabelSafely?.(labelEl, newIso);
    } catch (_) {}
  }
  ...
  function loadStatsTable() {
    ...
    try {
      if (
        localStorage.getItem('feature:league_ui_store') === '1' &&
        typeof window.loadStatsViaStore === 'function'
      ) {
        _statsLoading = true;
        window
          .loadStatsViaStore()
    ...
    fetch('/api/leaderboard/goal-assist?limit=50') // берём с запасом, потом сами отсортируем и обрежем до 10
      .then(async r => {
        ...
        items.sort((a, b) => {
          ...
          const am = a.matches_played || 0,
            bm = b.matches_played || 0;
          if (am !== bm) {
            return am - bm;
          } // меньше матчей — выше
          const ag = a.goals || 0,
        ...
        try {
          window.League?.renderStatsTable?.(table, updated, payload);
        } catch (_) {}
      })
      .catch(err => {
        console.error('stats table load error (leaderboard)', err);
      })
      .finally(() => {
        _statsLoading = false;
        if (tbody) {
  ...
  function ensureLeaderboardInit() {
    ...
    try {
      ...
      if (badge) {
        ...
        const monthStartMsk = new Date(
          Date.UTC(mskNow.getUTCFullYear(), mskNow.getUTCMonth(), 1, 3, 0, 0)
        );
        // Конец месяца: начало следующего месяца минус 1 день
        const nextMonthStartMsk = new Date(
          Date.UTC(mskNow.getUTCFullYear(), mskNow.getUTCMonth() + 1, 1, 3, 0, 0)
        );
        ...
        setTimeout(() => {
          const tryPrefetch = (cacheKey, url, swrMs) => {
            try {
              ...
              if (cached && cached.ts && Date.now() - cached.ts < swrMs) {
                return;
              } // свежо
            } catch (_) {}
      ...
      const tick = async () => {
        try {
          if (isPaneVisible(lbActiveTab)) {
            if (lbActiveTab === 'predictors') {
              loadLBPredictors({ forceRevalidate: true, skipIfNotUpdated: true });
            } else if (lbActiveTab === 'rich') {
              loadLBRich({ forceRevalidate: true, skipIfNotUpdated: true });
            } else if (lbActiveTab === 'server') {
              loadLBServer({ forceRevalidate: true, skipIfNotUpdated: true });
            }
    ...
    tabs.forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (key === 'predictors' || key === 'rich' || key === 'server') {
          startLbPolling(key);
        } else {
          stopLbPolling();
        }
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        ...
        if (visibleKey === 'predictors' || visibleKey === 'rich' || visibleKey === 'server') {
          startLbPolling(visibleKey);
        }
      }
    });
  }
  ...
  function loadLBPredictors(opts) {
    ...
    if (window.fetchEtag) {
      window
        .fetchEtag('/api/leaderboard/top-predictors', {
        ...
        .then(({ data, updated: isUpdated, headerUpdatedAt }) => {
          if (skipIfNotUpdated && !isUpdated) {
            if (updated && headerUpdatedAt) {
              try {
                updated.textContent = `Обновлено: ${new Date(headerUpdatedAt).toLocaleString()}`;
              } catch (_) {}
            }
          ...
          if (updated) {
            ...
            if (iso) {
              try {
                updated.textContent = `Обновлено: ${new Date(iso).toLocaleString()}`;
              } catch (_) {}
            }
          }
        })
      ...
      etagFetch('/api/leaderboard/top-predictors', 'lb:predictors')
        .then(store => {
          ...
          if (updated && store?.data?.updated_at) {
            try {
              updated.textContent = `Обновлено: ${new Date(store.data.updated_at).toLocaleString()}`;
            } catch (_) {}
          }
        })
        .catch(err => console.error('lb predictors err', err));
  ...
  function loadLBRich(opts) {
    ...
    if (window.fetchEtag) {
      window
        .fetchEtag('/api/leaderboard/top-rich', {
          cacheKey: 'lb:rich',
          swrMs: 60000,
          extract: j => j,
          forceRevalidate,
  ...
  function loadLBServer(opts) {
    ...
    if (window.fetchEtag) {
      window
        .fetchEtag('/api/leaderboard/server-leaders', {
          cacheKey: 'lb:server',
          swrMs: 60000,
          extract: j => j,
          forceRevalidate,
  ...
  function loadLBPrizes() {
    ...
    const pubCacheSet = (uid, data) => {
      try {
        localStorage.setItem('public:profile:' + uid, JSON.stringify({ ts: nowMs(), data }));
      } catch (_) {}
    };
    const tryFetchPublic = async ids => {
      try {
        const r = await fetch('/api/users/public-batch', {
          method: 'POST',
    ...
    const ensureOverlay = () => {
      ...
      if (!ov) {
        ...
        box.style.background = 'rgba(15,20,35,0.98)';
        box.style.border = '1px solid rgba(255,255,255,0.08)';
        box.style.borderRadius = '12px';
        box.style.padding = '14px';
        box.style.width = 'min(92%, 380px)';
        box.style.boxShadow = '0 20px 48px rgba(0,0,0,0.45)';
        ...
        cnt.className = 'pp-content';
        cnt.style.display = 'grid';
        cnt.style.gap = '10px';
        box.append(close, cnt);
        ov.appendChild(box);
    ...
    const renderProfileCard = (data, avatarUrl) => {
      ...
      const img = document.createElement('img');
      img.src = avatarUrl || '/static/img/achievements/placeholder.png';
      img.alt = '';
      ...
      name.textContent = data.display_name || 'Игрок';
      const meta = document.createElement('div');
      meta.style.fontSize = '12px';
      meta.style.color = 'var(--gray)';
      const parts = [];
      if (data.level != null) {
        parts.push(`Уровень ${data.level}`);
      }
      if (data.current_xp != null && data.next_xp != null) {
        parts.push(`${data.current_xp}/${data.next_xp} XP`);
      } else if (data.xp != null) {
        parts.push(`${data.xp} XP`);
      }
      if (data.consecutive_days != null) {
        parts.push(`Серия ${data.consecutive_days}`);
      }
    ...
    if (window.fetchEtag) {
      window
        .fetchEtag('/api/leaderboard/prizes', {
          cacheKey: 'lb:prizes',
          swrMs: 30000,
          extract: j => j,
        })
        .then(({ data, headerUpdatedAt }) => {
          ...
          blocks.forEach(b => {
            (dataBlock[b.key] || []).forEach(it => {
              if (it?.user_id) {
                allIds.add(it.user_id);
              }
            });
          });
          ...
          const render = avatars => {
            blocks.forEach(b => {
              ...
              order.forEach(i => {
                ...
                if (i === 0) {
                  pl.classList.add('gold');
                }
                if (i === 1) {
                  pl.classList.add('silver');
                }
                if (i === 2) {
                  pl.classList.add('bronze');
                }
                ...
                name.textContent = it ? it.display_name : '—';
                ...
                pl.addEventListener('click', async () => {
                  ...
                  if (!uid) {
                    renderProfileCard({ display_name: it?.display_name || 'Игрок' }, photo);
                    return;
                  }
                  const cached = pubCacheGet(uid);
                  if (cached) {
                    renderProfileCard(cached, photo);
                    return;
                  }
                  const map = await tryFetchPublic([Number(uid)]);
                  const dataPub = map[uid] || {
                    display_name: it?.display_name || 'Игрок',
                    level: it?.level,
                    xp: it?.xp,
                    consecutive_days: it?.consecutive_days,
                  };
          ...
          if (idsParam) {
            fetch(`/api/user/avatars?ids=${encodeURIComponent(idsParam)}`)
              .then(r => r.json())
              .then(d => {
                render(d.avatars || {});
              })
            ...
            blocks.forEach(b => {
              ...
              order.forEach(i => {
                ...
                const avatarUrl =
                  (it &&
                    avatars &&
                    avatars[String(it.user_id)] &&
                    avatars[String(it.user_id)].avatar_url) ||
                  '';
                pl.innerHTML = `
                  ...
                  renderProfileCard(
                    {
                      ...
                      level: prof?.level,
                      xp: prof?.xp,
                      consecutive_days: prof?.consecutive_days,
                    },
                    avatarUrl
                  );
                });
  ...
  function waitForSummary(ms = 1200) {
    return new Promise(resolve => {
      ...
      const cleanup = () => {
        ...
        try {
          window.removeEventListener('preload:summary-ready', onReady);
        } catch (_) {}
      };
      const onReady = () => {
        cleanup();
      ...
      try {
        window.addEventListener('preload:summary-ready', onReady, { once: true });
      } catch (_) {}
      timer = setTimeout(() => {
        cleanup();
        resolve('timeout');
  ...
  async function loadSchedule() {
    ...
    const renderSchedule = data => {
      try {
        window.League?.renderSchedule?.(pane, data?.data || data);
      } catch (_) {
    ...
    if (window.fetchEtag) {
      ...
      window
        ...
        .then(res => {
          ...
          const incomingTours = Array.isArray(scheduleData?.tours)
            ? scheduleData.tours
            : Array.isArray(scheduleData?.data?.tours)
              ? scheduleData.data.tours
              : [];
        ...
        .catch(err => {
          ...
          if (!cached && pane.childElementCount === 0) {
    ...
    const fetchWithETag = etag =>
      fetch('/api/schedule', { headers: etag ? { 'If-None-Match': etag } : {} }).then(async r => {
        if (r.status === 304 && cached) {
          return cached;
        }
        const data = await r.json();
        const version = data.version || r.headers.get('ETag') || null;
        const store = { data, version, ts: Date.now() };
        const incomingTours = Array.isArray(data?.tours)
          ? data.tours
  ...
  async function loadResults() {
    ...
    const renderResults = data => {
      try {
        window.League?.renderResults?.(pane, data?.data || data);
      } catch (_) {
    ...
    if (window.fetchEtag) {
      ...
      window
        ...
        .then(res => {
          ...
          const incoming = Array.isArray(resultsData?.results)
            ? resultsData.results
    ...
    const fetchWithETag = etag =>
      fetch('/api/results', { headers: etag ? { 'If-None-Match': etag } : {} }).then(async r => {
        if (r.status === 304 && cached) {
          return cached;
        }
        const data = await r.json();
    ...
    const go = store => {
      renderResults(store?.data || store);
      _resultsLoading = false;
      _resultsPreloaded = true;
      trySignalAllReady();
    };
  ...
  function preloadUfoData() {
    ...
    if (window.fetchEtag) {
      try {
        window.__SUMMARY_IN_FLIGHT__ = true;
        window.__SUMMARY_DONE__ = false;
      } catch (_) {}
      window
        .fetchEtag('/api/summary', {
        ...
        .then(store => {
          ...
          try {
            if (raw.schedule) {
              localStorage.setItem(
                'schedule:tours',
                JSON.stringify({ data: raw.schedule, version: null, ts: now })
              );
            }
          } catch (_) {}
          try {
            if (raw.results) {
              localStorage.setItem(
                'results:list',
                JSON.stringify({ data: raw.results, version: null, ts: now })
              );
            }
          } catch (_) {}
          try {
            if (raw.tours) {
              localStorage.setItem(
                'betting:tours',
                JSON.stringify({ data: raw.tours, version: null, ts: now })
              );
            }
          } catch (_) {}
          try {
            const lb = raw.leaderboard || {};
            if (lb['top-predictors']) {
              localStorage.setItem(
                'lb:predictors',
                JSON.stringify({
                  etag: null,
                  ts: now,
                  data: lb['top-predictors'],
                  raw: lb['top-predictors'],
                })
              );
            }
            if (lb['top-rich']) {
              localStorage.setItem(
                'lb:rich',
                JSON.stringify({ etag: null, ts: now, data: lb['top-rich'], raw: lb['top-rich'] })
              );
            }
            if (lb['server-leaders']) {
              localStorage.setItem(
                'lb:server',
                JSON.stringify({
                  etag: null,
                  ts: now,
                  data: lb['server-leaders'],
                  raw: lb['server-leaders'],
                })
              );
            }
            if (lb['prizes']) {
              localStorage.setItem(
                'lb:prizes',
                JSON.stringify({ etag: null, ts: now, data: lb['prizes'], raw: lb['prizes'] })
              );
            }
          } catch (_) {}
        })
        ...
        .finally(() => {
          _schedulePreloaded = true;
          trySignalAllReady();
          _resultsPreloaded = true;
          trySignalAllReady();
          try {
            window.__SUMMARY_IN_FLIGHT__ = false;
            window.__SUMMARY_DONE__ = true;
            window.dispatchEvent(new CustomEvent('preload:summary-ready'));
          } catch (_) {}
        });
    } else {
      ...
      if (window.__SUMMARY_IN_FLIGHT__ || cached) {
        ...
        window
          .fetchEtag('/api/schedule', { cacheKey: 'prewarm:schedule', extract: j => j?.data || j })
          .then(({ data, etag }) => {
            try {
              localStorage.setItem(
                'schedule:tours',
                JSON.stringify({ data, version: etag || null, ts: Date.now() })
              );
            } catch (_) {}
          })
          .finally(() => {
      ...
      fetch('/api/results', { headers: { 'Cache-Control': 'no-cache' } })
        .then(async r => {
          ...
          const version = data.version || r.headers.get('ETag') || null;
          try {
            localStorage.setItem('results:list', JSON.stringify({ data, version, ts: Date.now() }));
          } catch (_) {}
        })
        .finally(() => {
          _resultsPreloaded = true;
    ...
    try {
      ...
      if (!localStorage.getItem('betting:tours')) {
        fetch('/api/betting/tours', { headers: { 'Cache-Control': 'no-cache' } })
          .then(async r => {
            ...
            const store = { data, version, ts: Date.now() };
            try {
              localStorage.setItem('betting:tours', JSON.stringify(store));
            } catch (_) {}
          })
          .catch(() => {});
      }
      ...
      if (tg?.initDataUnsafe?.user) {
        ...
        fetch('/api/betting/my-bets', { method: 'POST', body: fd })
          ...
          .then(data => {
            try {
              localStorage.setItem('betting:mybets', JSON.stringify({ data, ts: Date.now() }));
            } catch (_) {}
          })
          .catch(() => {});
      }
  ...
  function setActiveLeague(code) {
    ...
    try {
      sessionStorage.setItem('activeLeague', code || 'UFO');
    } catch (_) {}
    try {
      updateNavLeagueIcon();
    } catch (_) {}
  ...
  function showLeagueOverlay() {
    ...
    if (!overlay || !ufoTabs || !ufoContent || !blbBlock) {
      return;
    }
    ...
    if (!overlay.__inited) {
      ...
      document.addEventListener('click', e => {
        ...
        if (e.target.closest('#league-overlay') || isUfoNav) {
          return;
        }
        overlay.style.display = 'none';
      });
  ...
  function selectUFOLeague(_silent, animate = false) {
    ...
    const ufoContent = document.getElementById('ufo-content');
    const blbBlock = document.getElementById('blb-block');
    if (!ufoTabs || !ufoContent || !blbBlock) {
      return;
    }
    setActiveLeague('UFO');
    // Apply UFO theme
  ...
  function selectBLBLeague(animate = false) {
    ...
    const showBLB = () => {
      initBLBSubtabs();
      window.removeEventListener('league:transition-covered', showBLB);
    };
  ...
  function playLeagueTransition(to) {
    try {
      ...
      if (to === 'BLB') {
        ...
        try {
          window.dispatchEvent(new CustomEvent('league:transition-covered', { detail: { to } }));
        } catch (_) {}
        setTimeout(() => {
          ...
          if (t) {
            t.textContent = 'Название лиги';
          }
          const logo = document.querySelector('.top-bar .league-logo');
          if (logo) {
            logo.src = '/static/img/placeholderlogo.png';
          }
          // Пауза 1s
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              layer.classList.remove('lt-unfill-top');
              try {
                window.dispatchEvent(new CustomEvent('league:transition-end', { detail: { to } }));
              } catch (_) {}
            }, 1000);
          }, 1000);
        }, 1000);
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              layer.style.display = 'none';
              layer.classList.remove('lt-unfill-bottom');
              try {
                window.dispatchEvent(new CustomEvent('league:transition-end', { detail: { to } }));
              } catch (_) {}
            }, 1000);
  ...
  function ensureLeagueShelf() {
    ...
    const mkTile = (code, icon, name) => {
      ...
      tile.className = 'league-tile';
      tile.setAttribute('data-league', code);
      const ic = document.createElement('div');
      ic.className = 'league-icon';
      ic.textContent = icon;
      const nm = document.createElement('div');
      nm.className = 'league-name';
      nm.textContent = name;
      tile.append(ic, nm);
    ...
    setTimeout(() => {
      const onDoc = e => {
        if (!shelf.contains(e.target) && !e.target.closest('nav.nav')) {
          closeLeagueShelf();
        }
      };
  ...
  function openLeagueDrawer() {
    ...
    if (nav) {
      nav.style.transition = 'transform .12s ease, opacity .12s ease';
  ...
  function showLeagueHint() {
    try {
      ...
      try {
        if (localStorage.getItem('hint:league-shown') === '1') {
          return;
        }
      } catch (_) {}
      if (document.getElementById('league-hint-tip')) {
        return;
      }
      ...
      requestAnimationFrame(() => {
        try {
          ...
          const clampedCenter = Math.max(
            margin + half,
            Math.min(window.innerWidth - margin - half, centerX)
          );
      ...
      const cleanup = () => {
        try {
          localStorage.setItem('hint:league-shown', '1');
        } catch (_) {}
        try {
          tip.remove();
        } catch (_) {}
  ...
  function renderEventsPane(host, match, cachedEvents) {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        scoreBox.style.border = '1px solid rgba(255,255,255,0.1)';
        ...
        try {
          if (window.MatchesStoreAPI && match.home && match.away) {
            ...
            if (mk) {
              ...
              if (entry?.score) {
                if (typeof entry.score.home === 'number') {
                  inpH.value = entry.score.home;
                }
                if (typeof entry.score.away === 'number') {
                  inpA.value = entry.score.away;
                }
              }
            }
            window.MatchesStoreAPI.subscribe(() => {
              try {
                ...
                if (mk2) {
                  ...
                  if (e2?.score) {
                    if (typeof e2.score.home === 'number' && inpH.value !== String(e2.score.home))
                      inpH.value = e2.score.home;
                    if (typeof e2.score.away === 'number' && inpA.value !== String(e2.score.away))
                      inpA.value = e2.score.away;
                  }
                }
              } catch (_) {}
            });
        ...
        btn.addEventListener('click', async () => {
          ...
          if (inpH.value !== '') {
            fd.append('score_home', inpH.value);
          }
          if (inpA.value !== '') {
            fd.append('score_away', inpA.value);
          }
          btn.disabled = true;
          ...
          try {
            ...
            if (!r.ok || d?.error) {
              throw new Error(d?.error || 'Ошибка сохранения счёта');
            }
            try {
              window.showAlert?.('Счёт сохранён', 'success');
            } catch (_) {}
          } catch (e) {
    ...
    const renderList = (listEl, items) => {
      ...
      items.forEach(e => {
        ...
        const typeLabel =
          { goal: 'Гол', assist: 'Пас', yellow: 'Желтая', red: 'Красная' }[e.type] || e.type;
    ...
    const applyData = data => {
      try {
        ...
        renderList(homeBlock.list, []);
        renderList(awayBlock.list, []);
      }
    };

    // начальный рендер
    if (cachedEvents) {
      applyData(cachedEvents);
    }
  ...
  try {
    setActiveLeague(getActiveLeague());
    updateNavLeagueIcon();
  } catch (_) {}
