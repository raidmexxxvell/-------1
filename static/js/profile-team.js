...
(function () {
  ...
  async function fetchOverview(teamName) {
    ...
    const { data, headerUpdatedAt } = await (
      window.fetchEtag || (u => fetch(u).then(r => r.json().then(j => ({ data: j }))))
    )(`/api/team/overview`, {
      cacheKey: key,
      swrMs: 60_000,
  ...
  async function fetchRoster(teamName) {
    ...
    const p = startFetch.then(({ data }) => {
      if (data && !data.error) {
        window.__teamRosterCache[key] = { ts: Date.now(), data };
      }
      return data;
    });
  ...
  function renderRoster(host, payload) {
    ...
    if (!list.length) {
      host.innerHTML = '<div style="padding:12px; color: var(--gray);">Состав пуст</div>';
      return;
    }
    ...
    headers.forEach(h => {
      ...
      th.style.background = 'var(--surface-alt,rgba(255,255,255,0.05))';
  ...
  function renderOverview(host, payload) {
    ...
    last5List.forEach(r => {
      ...
      scoreOverlay.className = 'score-badge';
      const badge = document.createElement('div');
      badge.className = 'badge';
      if (typeof r === 'string') {
        badge.className +=
          ' ' + (r === 'В' ? 'badge-win' : r === 'Н' ? 'badge-draw' : 'badge-loss');
        badge.textContent = r;
        dt.textContent = '';
        ...
        const rr = r?.result || 'Н';
        badge.className +=
          ' ' + (rr === 'В' ? 'badge-win' : rr === 'Н' ? 'badge-draw' : 'badge-loss');
        badge.textContent = rr;
        try {
          dt.textContent = r?.date ? new Date(r.date).toLocaleDateString() : '';
        } catch (_) {
          dt.textContent = '';
        }
    ...
    const centerX = 80,
      centerY = 10,
      radius = 55; // центр дуги ещё выше, радиус больше для лучшей видимости
    // Фон-трек
    ...
    const segments = [];
    if (wn > 0) {
      segments.push({ label: 'Победы', short: 'В', value: w, adj: wn, colorClass: 'seg-win' });
    }
    if (dn > 0) {
      segments.push({ label: 'Ничьи', short: 'Н', value: d, adj: dn, colorClass: 'seg-draw' });
    }
    if (ln > 0) {
      segments.push({ label: 'Поражения', short: 'П', value: l, adj: ln, colorClass: 'seg-loss' });
    }
    ...
    gLab.className = 'gauge-label';
    gLab.textContent = pluralMatches(total);
    ...
    const row = (label, value) => {
      ...
      d.innerHTML = `<span class="dot"></span>${label}: <b>${value}</b>`;
      return d;
    };
    ul.append(
      row('Победы', stats.wins || 0),
      row('Ничьи', stats.draws || 0),
      row('Поражения', stats.losses || 0)
    );
    ...
    const m = (icon, val, label) => {
      ...
      i.onerror = () => {
        try {
          i.onerror = null;
          i.src = '/static/img/icons/goal.png';
        } catch (_) {}
      };
    ...
    mini.append(
      ...
      m(
        ...
        payload?.cards && payload.cards.yellow ? payload.cards.yellow : 0,
        'Жёлтых'
      ),
      m(
        '/static/img/icons/red.png',
        payload?.cards && payload.cards.red ? payload.cards.red : 0,
        'Красных'
      ),
  ...
  async function openTeam(teamName) {
    ...
    try {
      document.getElementById('ufo-schedule')?.setAttribute('aria-hidden', 'true');
    } catch (_) {}
    ...
    [sched, res, table, stats].forEach(el => {
      if (el) {
    ...
    try {
      (window.setTeamLogo || window.TeamUtils?.setTeamLogo)?.(logo, teamName);
    } catch (_) {}
    ...
    try {
      ...
      if (rsPane) {
        try {
          rsPane.removeAttribute('data-loaded');
          rsPane.innerHTML = '';
        } catch (_) {}
        rsPane.setAttribute('style', 'display:none;');
      }
    } catch (_) {}
    // Загрузка Overview
    ...
    try {
      ...
      if (upd && (data?.updated_at || headerUpdatedAt)) {
        setUpdatedLabel(upd, data?.updated_at || headerUpdatedAt);
      }
    } catch (_e) {
      ov.innerHTML = '<div style="padding:12px; color: var(--danger);">Не удалось загрузить</div>';
    }
  }
            ...
            if (name) {
              ...
              if (fromMatch) {
                ...
                const subKey = activeSub
                  ? activeSub.getAttribute('data-subtab') || 'table'
                  : 'table';
              ...
              if (!isUfoVisible) {
                try {
                  document.querySelector('.nav-item[data-tab="ufo"]').click();
                } catch (_) {}
                setTimeout(() => {
                  try {
                    openTeam(name.trim());
        ...
        try {
          ...
          const target =
            document.querySelector(
              `#ufo-subtabs .subtab-item[data-subtab="${ctx.subtab || 'table'}"]`
      ...
      btn.addEventListener('click', () => {
        ...
        if (key === 'overview') {
          ...
          if (!rs.getAttribute('data-loaded')) {
            rs.innerHTML =
              '<div style="padding:12px; color: var(--gray);">Загружаю состав...</div>';
            const teamName = (document.getElementById('team-name') || {}).textContent || '';
            fetchRoster(teamName)
              .then(data => {
                if (data && !data.error) {
                  renderRoster(rs, data);
                  rs.setAttribute('data-loaded', '1');
                } else {
                  rs.innerHTML =
                    '<div style="padding:12px; color: var(--danger);">Не удалось загрузить</div>';
                }
                // SWR refresh параллельно (второй вызов fetchRoster отдаст cache сразу)
                setTimeout(() => {
                  fetchRoster(teamName).then(fresh => {
                    if (fresh && !fresh.error) {
                      renderRoster(rs, fresh);
                    }
                  });
                }, 10);
              })
              .catch(() => {
                rs.innerHTML =
                  '<div style="padding:12px; color: var(--danger);">Ошибка загрузки</div>';
              });
          }
        }
      });
