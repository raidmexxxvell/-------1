...
(function(){
  ...
  try { hLogo?.setAttribute('data-team-name', match.home||''); aLogo?.setAttribute('data-team-name', match.away||''); } catch(_) {}
    ...
    const wsTopic = (function(){ try { const h=(match?.home||'').toLowerCase().trim(); const a=(match?.away||'').toLowerCase().trim(); const raw=(match?.date?String(match.date):(match?.datetime?String(match.datetime):'')); const d=raw?raw.slice(0,10):''; return `match:${h}__${a}__${d}:details`; } catch(_) { return null; } })();
    const isWsActive = ()=>{ try { if(!window.__WEBSOCKETS_ENABLED__) return false; if(!wsTopic) return false; if (window.__WEBSOCKETS_CONNECTED && window.__WS_TOPIC_SUBSCRIBED?.has?.(wsTopic)) return true; const ru=window.realtimeUpdater; return !!(ru && ru.getTopicEnabled?.() && ru.hasTopic?.(wsTopic)); } catch(_) { return false; } };
