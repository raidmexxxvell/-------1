...
(function () {
  ...
  function withTeamCount(n) {
    try {
      return window.withTeamCount ? window.withTeamCount(n) : n;
    } catch (_) {
      return n;
    }
  }
  function openMatchScreen(match, details) {
    ...
    try {
      ['ufo-table', 'ufo-stats', 'ufo-results', 'ufo-schedule'].forEach(id => {
    ...
    if (score) {
      ...
      try {
        ...
        /* избегаем принудительного плейсхолдера */
      }
    }
    try {
      if (dt) {
    ...
    statsPane.style.display = 'none';
    function simpleRoster(pane, list) {
      pane.innerHTML = '';
    ...
    try {
      ...
      simpleRoster(homePane, homeList);
      simpleRoster(awayPane, awayList);
    } catch (_) {
      simpleRoster(homePane, []);
      simpleRoster(awayPane, []);
    }
    // Стор-ориентированное обновление счёта: используем общий адаптер
    try {
      window.ScoreDOMAdapter?.attach &&
        window.ScoreDOMAdapter.attach(score, { home: match.home, away: match.away });
    } catch (_) {}
