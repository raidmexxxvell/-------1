...
(function(){
  ...
  try { hLogo?.setAttribute('data-team-name', match.home||''); aLogo?.setAttribute('data-team-name', match.away||''); } catch(_) {}
    ...
    let scorePoll=null;
    const applyScore=(sh,sa)=>{ try { if (sh==null||sa==null) {return;} const txt=`${Number(sh)} : ${Number(sa)}`; if(score.textContent!==txt) { score.textContent=txt; } try { const keyCache=(window.MatchUtils? window.MatchUtils.matchKey(match): null) || window.__CURRENT_MATCH_KEY__; window.MatchState?.set && window.MatchState.set(keyCache,{ score: txt }); } catch(_){} } catch(_) {} };
