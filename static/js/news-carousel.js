...
(function(){
  ...
  function ensureModal(){
    ...
    // close handlers
    const close = ()=>{ try{ modal.classList.remove('show'); modal.style.display='none'; modal.setAttribute('aria-hidden','true'); }catch(_){} };
    modal.querySelector('#news-modal-close').addEventListener('click', close);
    modal.querySelector('.modal-backdrop').addEventListener('click', (e)=>{ if (e.target?.dataset?.close) close(); });
  ...
  function initCarousel(){
    ...
    fetchNews(8).then(items=>{
      ...
      if (!slides.length){
        if(list){ list.style.display=''; list.innerHTML='<div class="news-empty">Пока нет новостей</div>'; }
        try { window.dispatchEvent(new CustomEvent('main:news-ready')); } catch(_) {}
        try { document.dispatchEvent(new CustomEvent('main:news-ready')); } catch(_) {}
        return;
      }
      ...
      let index=0; let timer=null; let hovering=false;
      const apply=()=>{
