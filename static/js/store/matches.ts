...
(() => {
  ...
  function updateMatchStats(
    matchKey: string,
    stats: MatchStats & { __home?: string; __away?: string; __date?: string }
  ) {
    matches.update(state => {
      const cur = state.map[matchKey] || {
        info: null,
        score: null,
        events: [],
        stats: null,
      ...
      const home =
        stats && typeof stats.__home === 'string' ? stats.__home : cur.info?.home || null;
      const away =
        stats && typeof stats.__away === 'string' ? stats.__away : cur.info?.away || null;
      const date =
        stats && typeof stats.__date === 'string' ? stats.__date : cur.info?.date || undefined;
      cur.info =
        home || away ? { id: matchKey, home: home || '', away: away || '', date } : cur.info;
  ...
  function findMatchByTeams(home: string, away: string): string | null {
    ...
    for (const [key, entry] of Object.entries(state.map)) {
      ...
      console.log('[MatchesStore] Checking entry:', {
        key,
        entryHome,
        entryAway,
        match: entryHome === homeNorm && entryAway === awayNorm,
      });
  ...
  function addOrMergeMatch(payload: {
    home: string;
    away: string;
    date?: string;
    ...
    (window as any).MatchesStore.update((st: MatchesState) => {
      st.map[key] = {
        ...
        score: payload.score
          ? { home: Number(payload.score.home) || 0, away: Number(payload.score.away) || 0 }
          : null,
        events: [],
        stats: null,
        rosters: null,
        ui: {
          scoreText: payload.score
            ? `${Number(payload.score.home)} : ${Number(payload.score.away)}`
            : undefined,
        },
