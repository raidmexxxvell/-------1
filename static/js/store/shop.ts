...
(() => {
  const init: ShopState = { 
    ...
    orders: [], 
    products: [
      { id: 'boots', code: 'boots', name: 'Бутсы', price: 500, image: '/static/img/shop/boots.png' },
      { id: 'ball', code: 'ball', name: 'Мяч', price: 500, image: '/static/img/shop/ball.png' },
      { id: 'tshirt', code: 'tshirt', name: 'Футболка', price: 500, image: '/static/img/shop/tshirt.png' },
  ...
  const ShopHelpers = {
    addToCart: (item: { id: string; name: string; price: number; code?: string }) => {
      ...
      const existingIndex = state.cart.findIndex((cartItem: ShopCartItem) => cartItem.id === item.id);
    ...
    getCartTotal: (): number => {
      const state = shop.get();
      return state.cart.reduce((total: number, item: ShopCartItem) => total + (item.price * item.qty), 0);
    },
    ...
    placeOrder: async (): Promise<{ success: boolean; orderId?: string; error?: string }> => {
      try {
        ...
        // Валидация корзины
        const invalidItems = state.cart.filter((item: ShopCartItem) => !ShopHelpers.validateCartItem(item).valid);
        ...
        const order: ShopOrder = {
          ...
          createdAt: Date.now(),
          status: 'new',
          items_preview: state.cart.map((item: ShopCartItem) => `${item.name}×${item.qty}`).join(', ')
        };
