                    ...
                    try {
                        ...
                        if (typeof sh === 'number' && typeof sa === 'number') {
                            ...
                            const scoreEvent = new CustomEvent('matchScoreUpdate', {
                                detail: { home, away, score_home: sh, score_away: sa, timestamp: Date.now(), source: 'websocket' }
                            });
        ...
        try {
            ...
            if (p.entity === 'match_events' || p.entity === 'match_events_removed') {
                ...
                window.MatchesStore.update((s) => {
                    ...
                    const seen = new Set(eventsArray.map((ev) => `${ev.t}|${ev.kind}|${ev.team || ev.teamName || ev.side || ''}`));
            ...
            if (p.entity === 'match_rosters' || p.entity === 'rosters') {
                ...
                window.MatchesStore.update((s) => {
                    ...
                    if (fields.rosters) {
                        ...
                        if (fields.home_roster)
                            cur.rosters.home = fields.home_roster;
                        if (fields.away_roster)
                            cur.rosters.away = fields.away_roster;
                    }
                    cur.lastUpdated = Date.now();
                    s.map[key] = cur;
                });
