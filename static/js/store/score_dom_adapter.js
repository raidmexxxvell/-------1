...
(function () {
  ...
  function tryResolveKey(entry) {
    ...
    try {
      return window.MatchesStoreAPI.findMatchByTeams(entry.meta.home, entry.meta.away);
    } catch (_) {
      return null;
    }
  }
    ...
    try {
      storeUnsub = window.MatchesStoreAPI.subscribe(state => {
        registry.forEach(rec => {
          if (!rec.resolvedKey) {
            rec.resolvedKey = tryResolveKey(rec);
          }
          ...
          if (
            data &&
            data.score &&
            typeof data.score.home === 'number' &&
            typeof data.score.away === 'number'
          ) {
      ...
      if (st) {
        registry.forEach(r => {
          if (!r.resolvedKey) r.resolvedKey = tryResolveKey(r);
        });
      }
    } catch (e) {
      console.warn('[ScoreDOMAdapter] subscribe fail', e);
    }
  }
    ...
    attach(el, meta) {
      try {
        ...
        if (!registry.has(id)) {
          registry.set(id, {
            els: new Set(),
            meta: { home: meta.home, away: meta.away },
            resolvedKey: null,
            firstAt: Date.now(),
            fetched: false,
            scoreApplied: false,
        ...
        console.warn('[ScoreDOMAdapter] attach error', e);
      }
    },
  };
})();
