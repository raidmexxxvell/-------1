    ...
    if (cacheKey === 'league:schedule' && window.LeagueStore) {
      try {
        ...
        try {
          ...
          const changed =
            !prevMarker ||
            (typeof prevMarker.minTour === 'number' && typeof marker.minTour === 'number'
              ? marker.minTour < prevMarker.minTour
              : !!marker.minTour && prevMarker.minTour == null);
          if (changed && marker.minTour === 1) {
            // Очистка ключей голосования
            try {
              const keys = Object.keys(localStorage);
              for (const k of keys) {
                if (k.startsWith('voted:') || k.startsWith('voteAgg:')) localStorage.removeItem(k);
              }
            } catch (_) {
              /* noop */
            }
          ...
          try {
            localStorage.setItem('season:marker', JSON.stringify(marker));
          } catch (_) {
            /* noop */
          }
        } catch (_) {
        ...
        window.LeagueStore.update(s => {
          s.schedule.tours = tours;
          s.schedule.lastUpdated = Date.now();
          s.schedule.etag = etag ?? null;
        });
      } catch (_) {}
    }
      ...
      try {
        ...
        const table = Array.isArray((data as any)?.values)
          ...
          : Array.isArray(data)
            ...
            : (data as any)?.table || [];
        window.LeagueStore.update(s => {
          s.table = Array.isArray(table) ? table : [];
        });
      } catch (_) {}
    }
    ...
    if (cacheKey === 'predictions:list') {
      try {
        if (window.PredictionsStore) {
          ...
          window.PredictionsStore.update(s => {
            s.items = items;
            s.ttl = Date.now() + 5 * 60 * 1000;
          });
        }
            ...
            for (const o of arr) {
              ...
              const version =
                typeof o?.version === 'number'
                  ? o.version
                  : typeof o?.odds_version === 'number'
                    ? o.odds_version
                    : 0;
              const value =
                typeof o?.value === 'number' ? o.value : typeof o?.odds === 'number' ? o.odds : 0;
