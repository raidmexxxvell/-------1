...
(function () {
  ...
  const getLeagueUpdatedText = () =>
    document.getElementById('league-updated-text') as HTMLElement | null;
  ...
  const getStatsUpdated = () =>
    document.getElementById('stats-table-updated') as HTMLElement | null;
  ...
  function renderLeagueTableFromStore(state: LeagueState): void {
    ...
    try {
      ...
      const currentSig = (() => {
        try {
          return JSON.stringify(state.table.slice(0, 10));
        } catch {
          return null;
        }
      })();
  ...
  function renderScheduleFromStore(state: LeagueState): void {
    ...
    try {
      ...
      const data = {
        ...
        updated_at: state.schedule.lastUpdated
          ? new Date(state.schedule.lastUpdated).toISOString()
  ...
  function renderStatsFromStore(state: LeagueState): void {
    ...
    try {
      ...
      const currentSig = (() => {
        try {
          return JSON.stringify(state.stats.slice(0, 10));
        } catch {
          return null;
        }
      })();
      ...
      try {
        (table as any).dataset.sig = currentSig || '';
      } catch (_) {}
  ...
  function loadStatsViaStore(): Promise<void> {
    ...
    return window
      .fetchEtag('/api/leaderboard/goal-assist', {
  ...
  try {
    ...
    const hardDisable =
      /(?:[?&]|#)(?:ff|feature:league_ui_store)=0\b/.test(window.location.search) ||
