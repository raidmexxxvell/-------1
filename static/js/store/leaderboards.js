...
(function () {
  try {
    ...
    function sortLists(data) {
      if (Array.isArray(data.goals_assists))
        data.goals_assists.sort(
          (a, b) => b.total - a.total || a.games - b.games || a.player.localeCompare(b.player)
        );
      if (Array.isArray(data.goals))
        data.goals.sort(
          (a, b) => b.goals - a.goals || a.games - b.games || a.player.localeCompare(b.player)
        );
      if (Array.isArray(data.assists))
        data.assists.sort(
          (a, b) => b.assists - a.assists || a.games - b.games || a.player.localeCompare(b.player)
        );
    }
    ...
    function applyPatch(patch) {
      ...
      if (
        prevSig === newSig ||
        (!after.data.goals_assists?.length && (patch.goals_assists || patch.goals || patch.assists))
      ) {
    ...
    function fetchData(force) {
      ...
      if (!force && st.data.goals_assists && age < TTL) {
      ...
      if (!window.fetchEtag) {
        // fallback
        return fetch('/api/league/stats/leaderboards', {
          cache: 'no-store',
          headers: st.etag ? { 'If-None-Match': st.etag } : {},
        })
          ...
          .catch(e => {
            store.update(s => {
              s.loading = false;
              s.error = e.message || 'network';
            });
            return store.get();
          });
      }
      return window
        .fetchEtag('/api/league/stats/leaderboards', {
    ...
    document.addEventListener('leaderboards:patch', e => {
      try {
        ...
        console.warn('[LeaderboardsStore] patch event failed', err);
      }
    });
    ...
    if ('requestIdleCallback' in window) {
      requestIdleCallback(lazy, { timeout: 2000 });
      ...
      if (!hasWS) {
        setInterval(
          () => {
            try {
              const st = store.get();
              if (Date.now() - st.lastUpdated > TTL) {
                fetchData(false);
              }
            } catch (_) {}
          },
          Math.min(45_000, TTL)
        ); // проверяем чаще, но refetch только по TTL
      }
    } catch (_) {}

    console.log('[LeaderboardsStore] initialized', { cached: !!cached });
  } catch (e) {
    console.warn('[LeaderboardsStore] init failed', e);
  }
})();
