...
(function () {
  function initFallback() {
    if (window.__NAV_INITIALIZED__ || window.__NAV_FALLBACK_RAN__) {
      return;
    }
    const items = document.querySelectorAll('.nav-item');
    if (!items.length) {
      return;
    } // ничего не делать пока не появятся
      ...
      it.addEventListener(
        ...
        () => {
          try {
            ...
            ['home', 'ufo', 'predictions', 'leaderboard', 'shop', 'admin', 'profile'].forEach(
              id => {
                const el = document.getElementById('tab-' + id);
                if (el) {
                  el.style.display = tab === id ? '' : 'none';
                }
              }
            );
            window.scrollTo({ top: 0, behavior: 'instant' });
          } catch (e) {
            console.warn('Nav fallback click error', e);
          }
        },
        { passive: true }
      );
