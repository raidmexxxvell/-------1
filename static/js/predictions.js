...
(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function flattenAndUpdateOddsStoreForMatch(matchId, m, versionHint) {
      try {
        if (!window.OddsStore || !window.OddsStore.update) {
          return;
        }
        const now = Date.now();
        const v =
        ...
        if (m && m.odds) {
          if (m.odds.home != null) {
            apply(`${matchId}|1x2|home`, m.odds.home);
          }
          if (m.odds.draw != null) {
            apply(`${matchId}|1x2|draw`, m.odds.draw);
          }
          if (m.odds.away != null) {
            apply(`${matchId}|1x2|away`, m.odds.away);
          }
        }
        // Totals
        ...
        if (Array.isArray(totals)) {
          totals.forEach(row => {
            ...
            const over = row?.odds?.over;
            const under = row?.odds?.under;
            if (over != null) {
              apply(`${matchId}|totals|over|${line}`, over);
            }
            if (under != null) {
              apply(`${matchId}|totals|under|${line}`, under);
            }
          });
        }
        // Specials
        ...
        if (sp && sp.penalty && sp.penalty.odds) {
          const o = sp.penalty.odds;
          if (o.yes != null) {
            apply(`${matchId}|penalty|yes`, o.yes);
          }
          if (o.no != null) {
            apply(`${matchId}|penalty|no`, o.no);
          }
        }
        if (sp && sp.redcard && sp.redcard.odds) {
          const o = sp.redcard.odds;
          if (o.yes != null) {
            apply(`${matchId}|redcard|yes`, o.yes);
          }
          if (o.no != null) {
            apply(`${matchId}|redcard|no`, o.no);
          }
        }
      } catch (_) {}
    }
      ...
      try {
        ...
        tours.forEach(t =>
          (t.matches || []).forEach(m => {
            ...
            const totals = Array.isArray(m?.markets?.totals)
              ? m.markets.totals.map(r => r?.line).filter(l => l != null)
              : [];
            ...
            items.push({
              ...
              options: [
                has12 ? '1x2' : null,
                totals.length ? `totals(${totals.length})` : null,
                specials.length ? `specials(${specials.length})` : null,
              ].filter(Boolean),
            });
          })
        );
        ...
        if (markets && Array.isArray(markets.totals)) {
          markets.totals.forEach(row => {
            try {
              ...
              const overBtn = card.querySelector(
                `.bet-btn[data-market="totals"][data-side="over"][data-line="${line}"]`
              );
              const underBtn = card.querySelector(
                `.bet-btn[data-market="totals"][data-side="under"][data-line="${line}"]`
              );
          ...
          function updateOddsUIFromStore(store) {
            try {
              ...
              const cards = toursEl.querySelectorAll(
                '.pred-tours-container .match-card[data-match-id]'
              );
              cards.forEach(card => {
    ...
    pTabs.forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (pMap[key]) {
          ...
          if (key === 'mybets') {
            try {
              if (wrap.__oddsPollCancel) {
                wrap.__oddsPollCancel();
              }
            } catch (_) {}
            loadMyBets();
          }
    ...
    function loadTours() {
      ...
      const enqueueTopic = topic => {
        try {
          ...
          if (
            ...
            typeof window.realtimeUpdater.subscribeTopic === 'function'
          ) {
      ...
      const hasAnyOddsMarkets = store => {
        try {
          ...
          tours.forEach(t =>
            (t.matches || []).forEach(m => {
              if (m?.odds && Object.keys(m.odds).length) {
                hasOdds = true;
              }
              if (
                m?.markets &&
                ((Array.isArray(m.markets.totals) && m.markets.totals.length) || m.markets.specials)
              ) {
                hasMarkets = true;
              }
            })
            ...
            (t.matches || []).forEach(m => {
              try {
                ...
                if (!m.markets && prev.markets) {
                  m.markets = prev.markets;
                }
                // Если есть, но пусто — тоже дополним
                if (m.odds && prev.odds && Object.keys(m.odds).length === 0) {
                  m.odds = prev.odds;
                }
                if (m.markets && prev.markets) {
                  const mt = m.markets.totals;
                  const pm = prev.markets;
                  if (
                    !(Array.isArray(mt) && mt.length) &&
                    Array.isArray(pm.totals) &&
                    pm.totals.length
                  ) {
                    m.markets.totals = pm.totals;
                  }
                  if (!m.markets.specials && pm.specials) {
                    m.markets.specials = pm.specials;
                  }
                }
              } catch (_) {}
            })
      ...
      const renderTours = data => {
        ...
        if (!tours.length) {
          ...
          if (toursEl.childElementCount > 0 || toursEl.dataset.hasContent === '1') {
            return;
          }
        ...
        tours.forEach(t => {
          ...
          tourMatches.forEach(m => {
            ...
            try {
              ...
              const topic =
                scheme === 'with_date' ? `match_odds_${matchId}` : `match_odds_${m.home}_${m.away}`;
              enqueueTopic(topic);
            } catch (_) {}
            // --- КОНЕЦ НОВОГО КОДА ---
            ...
            const home = mkTeam(m.home);
            const score = document.createElement('div');
            score.className = 'score';
            score.textContent = 'VS';
            const away = mkTeam(m.away);
            ...
            try {
              if (window.VoteInline && typeof window.VoteInline.create === 'function') {
                const gv =
                  window.getTeamColor || (window.TeamUtils && window.TeamUtils.getTeamColor);
                const voteWidget = window.VoteInline.create({
                  home: m.home,
                  away: m.away,
                  date: (m.date || m.datetime || '').slice(0, 10),
                  getTeamColor: gv,
                });
                if (voteWidget) {
                  card.appendChild(voteWidget);
                }
              }
            } catch (_) {}

            const line = document.createElement('div');
            line.className = 'betting-line';
            ...
            moreBtn.textContent = 'Больше прогнозов';
            moreBtn.setAttribute('data-throttle', '800');
            const extra = document.createElement('div');
            extra.className = 'extra-markets hidden';
            ...
            if (totals.length) {
              ...
              totals.forEach(row => {
                ...
                rowEl.className = 'totals-row';
                const lbl = document.createElement('div');
                lbl.className = 'totals-line';
                lbl.textContent = `Тотал ${row.line.toFixed(1)}`;
                const btnOver = document.createElement('button');
                btnOver.className = 'bet-btn';
                btnOver.textContent = `Больше (${Number(row.odds.over).toFixed(2)})`;
                btnOver.dataset.market = 'totals';
                btnOver.dataset.side = 'over';
                btnOver.dataset.line = String(row.line);
                const btnUnder = document.createElement('button');
                btnUnder.className = 'bet-btn';
                btnUnder.textContent = `Меньше (${Number(row.odds.under).toFixed(2)})`;
                btnUnder.dataset.market = 'totals';
                btnUnder.dataset.side = 'under';
                btnUnder.dataset.line = String(row.line);
                btnOver.disabled = !!m.lock;
                btnUnder.disabled = !!m.lock;
                ...
                btnOver.addEventListener('click', () => {
                  ...
                  Promise.resolve(openStakeModal(t.tour, m, 'over', 'totals', row.line)).finally(
                    () => {
                ...
                btnUnder.addEventListener('click', () => {
                  if (btnUnder.disabled) {
                    return;
                  }
                  btnUnder.disabled = true;
                  Promise.resolve(openStakeModal(t.tour, m, 'under', 'totals', row.line)).finally(
                    () => {
            ...
            const mkYN = (title, odds, marketKey) => {
              ...
              yesBtn.className = 'bet-btn';
              yesBtn.textContent = `Да (${Number(odds.yes).toFixed(2)})`;
              yesBtn.dataset.market = marketKey;
              yesBtn.dataset.side = 'yes';
              const noBtn = document.createElement('button');
              noBtn.className = 'bet-btn';
              noBtn.textContent = `Нет (${Number(odds.no).toFixed(2)})`;
              noBtn.dataset.market = marketKey;
              noBtn.dataset.side = 'no';
              yesBtn.disabled = !!m.lock;
              noBtn.disabled = !!m.lock;
              ...
              yesBtn.addEventListener('click', () => {
                ...
                Promise.resolve(openStakeModal(t.tour, m, 'yes', marketKey)).finally(() => {
              ...
              noBtn.addEventListener('click', () => {
                ...
                noBtn.disabled = true;
                Promise.resolve(openStakeModal(t.tour, m, 'no', marketKey)).finally(() => {
      ...
      if (cached) {
        ...
        try {
          updateOddsUIFromStore(cached);
        } catch (_) {}
        // Обновим централизованный стор коэффициентов/предсказаний
        try {
          updateStoresFromTours(cached);
        } catch (_) {}
      } else {
      ...
      const fetchWithETag = etag =>
        fetch('/api/betting/tours', { headers: etag ? { 'If-None-Match': etag } : {} }).then(
          async r => {
            if (r.status === 304 && cached) {
              ...
              try {
                ...
                const hasOdds = tours.some(t =>
                  (t.matches || []).some(m => m?.odds && Object.keys(m.odds).length)
                );
                const hasMarkets = tours.some(t =>
                  (t.matches || []).some(
                    m => m?.markets && (m.markets.totals?.length || m.markets.specials)
                  )
                );
            ...
            const incoming = Array.isArray(data?.tours)
              ? data.tours
            ...
            if (cached) {
              store = mergeOddsMarkets(cached, store);
            }
            const writeable =
              incoming.length > 0 &&
              (hasAnyOddsMarkets(store) || !cached || cachedTours.length === 0);
            if (writeable) {
              writeCache(store);
            }
            try {
              updateStoresFromTours(store);
            } catch (_) {}
            return store;
          }
        );
      ...
      const startOddsPolling = initialVersion => {
        ...
        try {
          wsConnected = !!(
            ...
            window.realtimeUpdater &&
            window.realtimeUpdater.getConnectionStatus &&
            window.realtimeUpdater.getConnectionStatus().connected
          );
          topicsOn = !!(
            window.realtimeUpdater &&
            typeof window.realtimeUpdater.getTopicEnabled === 'function' &&
            window.realtimeUpdater.getTopicEnabled()
          );
          hasPredTopic = !!(
            window.realtimeUpdater &&
            typeof window.realtimeUpdater.hasTopic === 'function' &&
            window.realtimeUpdater.hasTopic('predictions_page')
          );
        } catch (_) {}
        const needPolling =
          !window.__WEBSOCKETS_ENABLED__ || !wsConnected || !topicsOn || !hasPredTopic;
        if (!needPolling) {
          return;
        }
        let cancelled = false,
        ...
        const schedule = () => {
          ...
          const base = 3500,
            jitter = 1200;
          timer = setTimeout(loop, base + Math.floor(Math.random() * jitter));
        };
        const loop = async () => {
          if (cancelled) {
            return;
          ...
          const placeVisible =
            pMap.place && pMap.place.style.display !== 'none' && wrap.style.display !== 'none';
          if (document.hidden || !placeVisible) {
            schedule();
            return;
          }
          if (busy) {
      ...
      const __isFresh__ =
        ...
        Date.now() - cached.ts < __FRESH_TTL__ &&
      ...
      if (cached && !_toursLoading) {
        /* уже отрисовали кэш; загрузка в фоне */
      }
    }
    ...
    function mkOptions(tour, m, locked) {
      ...
      const mkBtn = (key, label) => {
        ...
        b.addEventListener('click', () => {
          ...
          b.disabled = true;
          Promise.resolve(openStakeModal(tour, m, key)).finally(() => {
            b.disabled = false;
          });
        });
        return b;
    ...
    function openStakeModal(tour, m, selection, market = '1x2', line = null) {
      ...
      if (market === '1x2') {
        selText = { home: 'П1', draw: 'Х', away: 'П2' }[selection] || selection;
      } else if (market === 'totals') {
        selText =
          selection.startsWith('over') || selection.startsWith('under')
            ? selection.startsWith('over')
              ? `Больше ${selection.split('_')[1]}`
              : `Меньше ${selection.split('_')[1]}`
            : selText;
      } else if (market === 'penalty' || market === 'redcard') {
        selText = { yes: 'Да', no: 'Нет' }[selection] || selection;
      }
      // Покажем кастомную модалку вместо prompt
      return showStakeModal(
        `Ставка на ${m.home} vs ${m.away}`,
        `Исход: ${selText}. Введите сумму:`,
        '100'
      ).then(stake => {
        if (!stake) {
        ...
        return fetch('/api/betting/place', { method: 'POST', body: fd })
          ...
          .then(resp => {
            ...
            try {
              tg?.showAlert?.(`Ставка принята! Баланс: ${resp.balance}`);
            } catch (_) {}
            // Обновим профильные кредиты на экране
            const creditsEl = document.getElementById('credits');
            if (creditsEl) {
              creditsEl.textContent = (resp.balance || 0).toLocaleString();
            }
          ...
          .catch(err => {
            try {
              tg?.showAlert?.('Ошибка размещения ставки');
            } catch (_) {}
          });
      });
    }
    ...
    function showStakeModal(title, message, defaultValue) {
      return new Promise(resolve => {
        ...
        modal.className = 'modal stake-modal show';
    ...
    function loadMyBets() {
      ...
      if (!tg || !tg.initDataUnsafe?.user) {
        myBetsEl.textContent = 'Недоступно вне Telegram';
        return;
      }
      ...
      const render = data => {
        ...
        bets.forEach(b => {
          ...
          title.textContent = `${b.home} vs ${b.away}`;
          const when = document.createElement('div');
          when.className = 'bet-when';
          when.textContent = b.datetime ? formatDateTime(b.datetime) : '';
          ...
          mid.textContent = `${marketDisp}: ${selDisp} | Кф: ${b.odds || '-'} | Ставка: ${b.stake}`;
      ...
      fetch('/api/betting/my-bets', { method: 'POST', body: fd })
        ...
        .catch(err => {
          if (!cachedBets) {
            myBetsEl.innerHTML = '<div class="schedule-error">Ошибка загрузки</div>';
          }
        });
    }
    ...
    const formatDateTime = (d, t) =>
      window.MatchUtils ? window.MatchUtils.formatDateTime(d, t) : d || '';
    ...
    document.addEventListener('bettingOddsUpdate', e => {
      ...
      if (!card) {
        card = document.querySelector(
          `.match-card[data-home="${homeTeam}"][data-away="${awayTeam}"]`
        );
      }
      if (!card) {
        return;
            ...
            (t.matches || []).forEach(m => {
              ...
              if (m.home === homeTeam && m.away === awayTeam && md === d) {
                ...
                if (markets && Object.keys(markets).length) {
                  m.markets = { ...(m.markets || {}), ...markets };
                }
                if (detail.odds_version != null) {
                  cached.version = String(detail.odds_version);
                }
              }
            })
          );
