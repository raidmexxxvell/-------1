...
(function () {
    ...
    try {
      ...
      try {
        ...
        if (resultsCache?.data?.results) {
          const found = resultsCache.data.results.find(r => r.home === m.home && r.away === m.away);
  ...
  const MatchNotifications = {
    ...
    showMatchStart(match) {
      ...
      try {
        if (!document.getElementById('match-start-banner')) {
          ...
          div.style.cssText =
            'position:fixed;top:12px;left:50%;transform:translateX(-50%);background:#1976d2;color:#fff;padding:10px 16px;border-radius:10px;z-index:10000;font-size:14px;box-shadow:0 6px 20px rgba(0,0,0,.35);cursor:pointer;';
    ...
    scan(matches) {
      try {
        ...
        (matches || []).forEach(m => {
          if (isLiveNow(m, { nowTs })) {
            this.showMatchStart(m);
          }
        });
      } catch (_) {}
    },
  ...
  try {
    window.isLiveNow = isLiveNow;
  } catch (_) {}
  try {
    window.formatMatchDateTime = formatDateTime;
  } catch (_) {}
})();
