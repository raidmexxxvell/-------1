...
(function () {
  ...
  const _rIC =
    ...
    function (cb) {
      return setTimeout(() => cb({ timeRemaining: () => 0 }), 0);
    };
  ...
  function normalizeDateStr(raw) {
    try {
      ...
      if (tIdx > 0 && /^\d{4}-\d{2}-\d{2}$/.test(s.slice(0, 10))) {
        return s.slice(0, 10);
      }
  ...
  const VoteAgg = (() => {
    ...
    const keyFrom = (home, away, dateStr) => {
      try {
        ...
        return `${(home || '').toLowerCase()}__${(away || '').toLowerCase()}__${normalizeDateStr(dateStr || '')}`;
      }
    };
    const readCache = key => {
      try {
        const j = JSON.parse(localStorage.getItem('voteAgg:' + key) || 'null');
        if (j && Number.isFinite(j.ts) && Date.now() - j.ts < TTL) {
          return j;
        }
      } catch (_) {}
      return null;
    ...
    const writeCache = (key, data) => {
      try {
        localStorage.setItem('voteAgg:' + key, JSON.stringify({ ...data, ts: Date.now() }));
      } catch (_) {}
    };
    const pump = () => {
      ...
      try {
        ...
        if (init) {
          url += `&initData=${encodeURIComponent(init)}`;
        }
      } catch (_) {}
    ...
    const fetchAgg = (home, away, dateStr) => {
      ...
      const p = new Promise(res => {
        q.push({ key, h: home || '', a: away || '', d: String(dateStr || '').slice(0, 10), res });
        pump();
      });
      dedup.set(key, p);
  ...
  function renderLeagueTable(tableEl, updatedTextEl, data) {
    ...
    try {
      ...
      if (sameSig && notNewer) {
        return;
      } // ничего не поменялось — не перерисовываем
      if (tableEl.dataset) {
        tableEl.dataset.sig = sigRows;
      }
    } catch (_) {}
    ...
    raf(() => {
      try {
        if (updatedTextEl && data?.updated_at) {
          updatedTextEl.textContent = `Обновлено: ${new Date(data.updated_at).toLocaleString()}`;
        }
      } catch (_) {}
    });
  }
  ...
  function renderSchedule(pane, data) {
    ...
    try {
      const mkKeySafe = m => {
        try {
          ...
          return `${m?.home || ''}__${m?.away || ''}__${m?.date || m?.datetime || ''}`;
        }
      };
      const sig = JSON.stringify(
        (Array.isArray(tours) ? tours : []).map(t => ({
          t: t?.tour || t?.title || '',
    ...
    tours = Array.isArray(tours)
      ? tours.filter(t => Array.isArray(t.matches) && t.matches.length > 0)
      : [];
    if (!tours.length) {
    ...
    function createMatchCard(m) {
      ...
      try {
        if (m && typeof m.home === 'string') {
          card.setAttribute('data-match-home', m.home);
        }
        if (m && typeof m.away === 'string') {
          card.setAttribute('data-match-away', m.away);
        }
      } catch (_) {}
      ...
      const dateStr = (() => {
        try {
          const ds = normalizeDateStr(m.date || m.datetime);
          return ds ? formatDateRu(ds) : '';
        } catch (_) {
          return '';
        }
      })();
      ...
      let isLive = false;
      try {
        if (window.MatchUtils) {
          isLive = window.MatchUtils.isLiveNow(m);
        }
      } catch (_) {}
      ...
      try {
        card.setAttribute('data-date', normalizeDateStr(m.date || m.datetime) || '');
      } catch (_) {}
      try {
        if (m.time) {
          card.setAttribute('data-time', String(m.time));
        }
      } catch (_) {}
      ...
      hImg.className = 'logo';
      hImg.alt = m.home || '';
      hImg.setAttribute('data-team-name', m.home || '');
      try {
        hImg.loading = 'lazy';
        hImg.decoding = 'async';
      } catch (_) {}
      ...
      hName.className = 'team-name';
      hName.setAttribute('data-team-name', m.home || '');
      hName.textContent = withTeamCount(m.home || '');
      home.append(hImg, hName);
      // Добавляем класс match-score, чтобы realtime-updates смог находить и обновлять счёт по WS
      const score = document.createElement('div');
      score.className = 'score match-score';
      ...
      aImg.className = 'logo';
      aImg.alt = m.away || '';
      aImg.setAttribute('data-team-name', m.away || '');
      try {
        aImg.loading = 'lazy';
        aImg.decoding = 'async';
      } catch (_) {}
      ...
      aName.className = 'team-name';
      aName.setAttribute('data-team-name', m.away || '');
      aName.textContent = withTeamCount(m.away || '');
      ...
      try {
        if (isLive && !finStore[mkKey(m)]) {
          if (!/\d+\s*:\s*\d+/.test((score.textContent || '').trim())) {
            score.textContent = '0 : 0';
          }
          window.ScoreDOMAdapter?.attach &&
            window.ScoreDOMAdapter.attach(score, { home: m.home, away: m.away });
          try {
            window.MatchesStoreAPI?.addOrMergeMatch &&
              window.MatchesStoreAPI.addOrMergeMatch({
                home: m.home || '',
                away: m.away || '',
                date: (m.date || m.datetime || '').toString().slice(0, 10),
                score: {
                  home: Number(score.textContent.split(':')[0]) || 0,
                  away: Number(score.textContent.split(':')[1]) || 0,
                },
              });
          } catch (_) {}
      ...
      try {
        ...
        if (adminId && currentId && adminId === currentId) {
          ...
          const isFeatured = (() => {
            try {
              const s = localStorage.getItem('feature:current');
              if (!s) {
                return false;
              }
              const j = JSON.parse(s);
              return j && j.home === m.home && j.away === m.away;
            } catch (_) {
              return false;
            }
          })();
          ...
          star.addEventListener('click', async () => {
            try {
              ...
              if (m.date) {
                fd.append('date', String(m.date).slice(0, 10));
              }
              if (m.datetime) {
                fd.append('datetime', String(m.datetime));
              }
              ...
              star.textContent = 'Закреплено';
              try {
                localStorage.setItem(
                  'feature:current',
                  JSON.stringify({ home: m.home || '', away: m.away || '', ts: Date.now() })
                );
              } catch (_) {}
              try {
                window.renderTopMatchOfWeek?.({
                  home: m.home,
                  away: m.away,
                  date: m.date,
                  datetime: m.datetime,
              ...
              try {
                document.dispatchEvent(
                  new CustomEvent('feature-match:set', { detail: { match: m } })
                );
              } catch (_) {}
            } catch (_) {
              try {
                window.Telegram?.WebApp?.showAlert?.('Не удалось назначить матч недели');
              } catch (_) {}
            }
          });
      ...
      btnDetails.addEventListener('click', () => {
        ...
        btnDetails.textContent = 'Загрузка контента...';
        const finish = store => {
          try {
          ...
          btnDetails.textContent = original;
        };
    ...
    holders.slice(0, INITIAL_RENDER).forEach(h => {
      renderMatchesInto(h.querySelector('.tour-body'), h.__matches);
      h.__rendered = true;
    });
            ...
            if (ent.isIntersecting && !el.__rendered) {
              ...
              if (visible.size > MAX_RENDERED_TOURS) {
                ...
                if (toRemove && toRemove !== el) {
                  const body = toRemove.querySelector('.tour-body');
                  if (body) {
                    body.innerHTML = '';
                  }
                  toRemove.__rendered = false;
                  visible.delete(toRemove);
                }
              }
            }
          });
    ...
    try {
      if (pane.__liveRescanTimer) {
        clearInterval(pane.__liveRescanTimer);
      }
      ...
      const LiveStatusCache = (window.__LIVE_STATUS_CACHE = window.__LIVE_STATUS_CACHE || {
        ...
        set(k, status) {
          this.map.set(k, { ts: Date.now(), status });
        },
      });

      const rescanLiveBadges = () => {
        try {
          if (!window.MatchUtils || typeof window.MatchUtils.isLiveNow !== 'function') {
            return;
          }
          const finStore = (window.__FINISHED_MATCHES = window.__FINISHED_MATCHES || {});
          const mkKey = mm => {
            try {
              return `${(mm.home || '').toLowerCase().trim()}__${(mm.away || '').toLowerCase().trim()}__${(mm.date || mm.datetime || '').toString().slice(0, 10)}`;
            } catch (_) {
              return `${mm.home || ''}__${mm.away || ''}`;
            }
          };
          ...
          cards.forEach(card => {
            ...
            try {
              ...
              const home =
                card.querySelector('.team.home .team-name')?.getAttribute('data-team-name') ||
                card.querySelector('.team.home .team-name')?.textContent ||
                '';
              const away =
                card.querySelector('.team.away .team-name')?.getAttribute('data-team-name') ||
                card.querySelector('.team.away .team-name')?.textContent ||
                '';
                    ...
                    if (diffMin <= 130) {
                      ...
                      if (cached === 'live') {
                        ...
                        setTimeout(() => {
                          ...
                          if (LiveStatusCache.get(key) !== null) {
                            return;
                          }
                          fetch(
                            `/api/match/status/get?home=${encodeURIComponent(matchObj.home || '')}&away=${encodeURIComponent(matchObj.away || '')}`
                          )
                            .then(r => r.json())
                            .then(s => {
                              try {
  ...
  function renderResults(pane, data) {
    ...
    const renderPage = () => {
      ...
      matches.sort((m1, m2) => {
        const d1 = m1.datetime || m1.date || '';
        const d2 = m2.datetime || m2.date || '';
      ...
      prev.disabled = idx <= 0;
      next.disabled = idx >= tourList.length - 1;
    };
    prev.onclick = () => {
      if (idx > 0) {
        idx--;
  ...
  function refreshTable() {
    try {
      ...
      fetch('/api/league-table/live', { cache: 'no-store' })
        ...
        .then(data => {
          try {
            renderLeagueTable(table, updatedText, data);
          } catch (_) {}
          ...
          window
              ...
              onSuccess: res => {
                try {
                  if (window.LeagueStore) {
                    window.LeagueStore.update(s => {
                      s.table = Array.isArray(res.data) ? res.data : [];
                    });
                  }
                } catch (_) {}
              },
            ...
            .then(({ data, headerUpdatedAt }) => {
              ...
              if (updatedText && headerUpdatedAt) {
                try {
                  setUpdatedLabelSafely(updatedText, headerUpdatedAt);
                } catch (_) {}
              }
            })
            .catch(() => {});
  ...
  async function refreshSchedule() {
    try {
      ...
      let cached = null;
      try {
        cached = JSON.parse(localStorage.getItem(STORE_KEY) || 'null');
      } catch (_) {
        cached = null;
      }
      const isFresh =
        ...
        Date.now() - cached.ts < FRESH_TTL &&
        ((cached.data?.tours && cached.data.tours.length > 0) ||
          (cached?.tours && cached.tours.length > 0));
      if (isFresh) {
        try {
          renderSchedule(pane, cached.data || cached);
        } catch (_) {}
        ...
        .fetchEtag('/api/schedule', {
          ...
          onSuccess: res => {
            try {
              if (window.LeagueStore) {
                window.LeagueStore.update(s => {
                  s.schedule.tours = Array.isArray(res.data?.tours)
                    ? res.data.tours
  ...
  if (typeof renderStatsTable !== 'function') {
    var renderStatsTable = function (tableEl, updatedEl, data) {
      try {
        /* no-op fallback */
      } catch (_) {}
    };
  ...
  function initLeagueStatsSwiper() {
    ...
    function updateNav() {
      ...
      if (title)
        title.innerHTML =
          '<span class="ls-arrows">⟵</span> ' + TITLES[idx] + ' <span class="ls-arrows">⟶</span>';
    }

    function buildTable(list, updatedAt) {
      const wrap = document.createElement('div');
      wrap.className = 'league-table-wrap';
      const table = document.createElement('table');
      table.className = 'league-table';
      const thead = document.createElement('thead');
      thead.innerHTML =
        '<tr><th>#</th><th>Игрок</th><th>Команда</th><th>И</th><th>Г</th><th>П</th><th>Г+П</th></tr>';
      ...
      for (let i = 0; i < 10; i++) {
        ...
        for (let c = 0; c < 7; c++) {
          ...
          td.innerHTML = '<span class="sk-bar"></span>';
          tr.appendChild(td);
        }
      ...
      upd.className = 'table-updated';
      upd.textContent = updatedAt ? 'Обновлено: ' + new Date(updatedAt).toLocaleString() : '';
      ...
      wrap.__applyData = function (list2, updatedAt2) {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          // If skeleton, clear
          if (tr.classList.contains('sk-row')) {
            tr.className = '';
            tr.innerHTML = '';
          }
          const desired = r
        ...
        upd.textContent = updatedAt2 ? 'Обновлено: ' + new Date(updatedAt2).toLocaleString() : '';
      };
      return wrap;
    }

    const visited = new Set(['ga']); // ленивый рендер: первый слайд сразу, остальное по первому заходу
    ...
    function renderSlides() {
      ...
      slides.forEach(sl => {
        ...
        if (!visited.has(type)) return; // не посещён ещё → не рендерим
        let list = null;
        ...
        if (sl.__tableWrap && sl.__tableWrap.__applyData) {
          sl.__tableWrap.__applyData(list, st.updatedAt);
        }
    ...
    function go(n) {
      ...
      const code = map[idx];
      try {
        window.LeaderboardsStoreAPI?.setActiveTab(storeMap[code]);
      } catch (_) {}
      visited.add(code);
      renderSlides();
    }
    ...
    (function waitStore() {
      if (window.LeaderboardsStore) {
        try {
          window.LeaderboardsStore.subscribe(() => {
            renderSlides();
          });
        } catch (_) {}
        try {
          window.LeaderboardsStoreAPI?.ensureFresh(true);
        } catch (_) {}
        renderSlides();
      } else {
  ...
  function readToursCache() {
    try {
      return JSON.parse(localStorage.getItem(TOURS_CACHE_KEY) || 'null');
    } catch (_) {
      return null;
    }
  }
  function writeToursCache(obj) {
    try {
      localStorage.setItem(TOURS_CACHE_KEY, JSON.stringify(obj));
    } catch (_) {}
  }
  function ensureBettingToursFresh() {
    try {
      ...
      const fresh =
        cached &&
        Number.isFinite(cached.ts) &&
        Date.now() - cached.ts < TOURS_FRESH_TTL &&
        (Array.isArray(cached?.data?.tours)
          ? cached.data.tours.length > 0
          : Array.isArray(cached?.tours) && cached.tours.length > 0);
      if (fresh) {
        return Promise.resolve(cached);
      }
      if (__toursFetchPromise) {
  ...
  function buildTourMatchKey(obj) {
    return matchKey(obj);
  }
  ...
  function patchScheduleVotes(pane) {
    /* no-op: голосование показываем только во вкладке «Прогнозы» */
  }
      ...
      try {
        ...
        document.querySelectorAll('.vote-inline[data-votekey]').forEach(wrap => {
          try {
            ...
            if (st && Date.now() - (st.lastAggTs || 0) < 3500) {
              return;
            } // ещё свежо
            ...
            VoteAgg.fetchAgg(home, away, date)
              .then(agg => {
  ...
  if (!window.__VOTE_POLLING_STARTED__) {
    window.__VOTE_POLLING_STARTED__ = true;
    startVotePolling();
  }
})();
  ...
  if (window.io && typeof window.io === 'function') {
    ...
    socket.on('connect', () => {
      try {
        ...
        stopToursPolling();
        console.info('WS connected for live updates');
      } catch (_) {}
    });
    ...
    socket.on('data_changed', msg => {
      try {
        if (!msg || msg.data_type !== 'betting_tours') {
          return;
        }
        // Сбрасываем локальный кэш и обновляем туры/расписание
        try {
          localStorage.removeItem('betting:tours');
        } catch (_) {}
        try {
