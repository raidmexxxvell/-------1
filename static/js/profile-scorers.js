...
(function () {
  ...
  function renderScorers(forceRefresh = false) {
    if (isStoreEnabled()) {
      return;
    } // do not interfere when store UI is on
    ...
    tbody.innerHTML =
      '<tr><td style="padding:8px; font-size:12px; opacity:.7;">Загрузка...</td></tr>';
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        if (!items.length) {
          tbody.innerHTML =
            '<tr><td style="padding:8px; font-size:12px; opacity:.7;">Нет данных</td></tr>';
          return;
        }
        // Header
        tbody.innerHTML = '';
        ...
        header.innerHTML =
          '<th style="text-align:left; padding:6px;">#</th><th style="text-align:left; padding:6px;">Игрок</th><th style="padding:6px;">И</th><th style="padding:6px;">Г</th><th style="padding:6px;">П</th><th style="padding:6px;">Г+П</th>';
        ...
        try {
          ...
          if (upd) {
            const dt = data.updated_at ? new Date(data.updated_at * 1000) : null;
            upd.textContent = 'Обновлено: ' + (dt ? dt.toLocaleString('ru-RU') : '—');
          }
        } catch (_) {}
      })
      .catch(err => {
        ...
        tbody.innerHTML =
          '<tr><td style="padding:8px; font-size:12px; opacity:.7;">Ошибка загрузки</td></tr>';
      });
  }
  ...
  document.addEventListener('click', e => {
    try {
      ...
      if (tab) {
        ...
        setTimeout(renderScorers, 50);
      }
    } catch (_) {}
  });
  // Фолбэк автозапуска (если уже открыта вкладка stats через программный клик)
  setTimeout(() => {
    const active = document.querySelector('#ufo-subtabs .subtab-item.active[data-subtab="stats"]');
    if (active) {
      if (!isStoreEnabled()) {
        renderScorers();
      }
