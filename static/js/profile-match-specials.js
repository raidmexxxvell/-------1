...
(function () {
  function render(host, match) {
    ...
    shell.style.border = '1px solid rgba(255,255,255,0.1)';
    shell.style.borderRadius = '10px';
    const title = document.createElement('div');
    title.style.marginBottom = '6px';
    title.textContent = 'Спецсобытия матча';
    ...
    sLab.textContent = 'Статус:';
    const sBadge = document.createElement('span');
    sBadge.className = 'status-badge';
    const updStatus = async () => {
      try {
        const r = await fetch(
          `/api/match/status/get?home=${encodeURIComponent(match.home || '')}&away=${encodeURIComponent(match.away || '')}`
        );
    ...
    lab1.textContent = 'Пенальти:';
    const sel1 = document.createElement('select');
    sel1.innerHTML =
      '<option value="">—</option><option value="1">Да</option><option value="0">Нет</option>';
    ...
    const savePenalty = document.createElement('button');
    savePenalty.className = 'app-btn neutral';
    savePenalty.textContent = 'Сохранить и рассчитать пенальти';
    const saveRed = document.createElement('button');
    saveRed.className = 'app-btn neutral';
    saveRed.textContent = 'Сохранить и рассчитать красную';
    const settleMatchBtn = document.createElement('button');
    settleMatchBtn.className = 'app-btn danger';
    settleMatchBtn.textContent = 'Рассчитать матч';
    ...
    try {
      host.__onclose = () => clearInterval(stId);
    } catch (_) {}
    fetch(
      `/api/specials/get?home=${encodeURIComponent(match.home || '')}&away=${encodeURIComponent(match.away || '')}`
    )
      .then(r => r.json())
      .then(d => {
        try {
    ...
    const saveAndSettle = async market => {
      ...
      if (market === 'penalty') {
        if (sel1.value === '') {
          window.showAlert?.('Укажите значение для Пенальти', 'warning');
          return;
        }
        fd.append('penalty_yes', sel1.value);
      } else if (market === 'redcard') {
        if (sel2.value === '') {
          window.showAlert?.('Укажите значение для Красной карточки', 'warning');
          return;
        }
      ...
      try {
        ...
        fd2.append('home', match.home || '');
        fd2.append('away', match.away || '');
        fd2.append('market', market);
        const r2 = await fetch('/api/specials/settle', { method: 'POST', body: fd2 });
        const d2 = await r2.json().catch(() => ({}));
        if (!r2.ok || d2?.error) {
          throw new Error(d2?.error || 'Ошибка расчёта');
        }
        window.showAlert?.(
          `Готово: изменено ${d2.changed || 0}, выиграло ${d2.won || 0}, проиграло ${d2.lost || 0}`,
          'success'
        );
      } catch (e) {
        console.error('specials save/settle error', e);
        window.showAlert?.(e?.message || 'Ошибка операции', 'error');
      } finally {
        btn.disabled = false;
        btn.textContent = old;
      }
    ...
    const confirmSettle = () =>
      new Promise(resolve => {
        ...
        if (!ov) {
          ...
          box.querySelector('#ms-ok').onclick = () => {
            ov.remove();
            resolve(true);
          };
        } else {
    ...
    settleMatchBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const d = await r.json().catch(() => ({}));
        if (!r.ok || d?.error) {
          throw new Error(d?.error || 'Ошибка расчёта матча');
        }
        window.showAlert?.(
          `Готово: изменено ${d.changed || 0}, выиграло ${d.won || 0}, проиграло ${d.lost || 0}`,
          'success'
        );
        try {
          ...
          const mkKey = mm => {
            try {
              const dstr = (mm?.datetime || mm?.date || '').toString().slice(0, 10);
              return `${(mm.home || '').toLowerCase().trim()}__${(mm.away || '').toLowerCase().trim()}__${dstr}`;
            } catch (_) {
              return `${(mm.home || '').toLowerCase().trim()}__${(mm.away || '').toLowerCase().trim()}__`;
            }
          };
          const keyFin = mkKey(match);
          finStore[keyFin] = true;
          try {
            document.dispatchEvent(
              new CustomEvent('app:match-finished', { detail: { key: keyFin, match } })
            );
          } catch (_) {}
        } catch (_) {}
      } catch (e) {
        console.error('match settle error', e);
        window.showAlert?.(e?.message || 'Ошибка расчёта', 'error');
      } finally {
        btn.disabled = false;
        btn.textContent = old;
      }
