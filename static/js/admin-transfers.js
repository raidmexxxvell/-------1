...
(function() {
    ...
    function renderPlayersTable(filteredPlayers = null) {
        ...
        playersToShow.forEach(player => {
            ...
            const inQueue = transferQueue.some(t => 
                t.player_name === player.full_name && t.from_team === player.team_name
            );
    ...
    function filterPlayers() {
        const searchTerm = document.getElementById('transfer-player-search')?.value?.toLowerCase() || '';
    ...
    function addToTransferQueue() {
        ...
        const existingTransfer = transferQueue.find(t => 
    ...
    async function executeTransfers() {
        ...
        try {
            ...
            if (successfulTransfers.length > 0 && failedTransfers.length === 0) {
                showNotification(`Все ${successfulTransfers.length} переводов выполнены успешно! Новость создана.`, 'success');
            } else if (successfulTransfers.length > 0) {
                showNotification(`${successfulTransfers.length} переводов выполнено успешно, ${failedTransfers.length} с ошибками.`, 'warning');
            } else {
